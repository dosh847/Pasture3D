...
[Tool]
public partial class Pasture3DData : GodotObject
{

	private new static readonly StringName NativeName = new StringName("Pasture3DData");

	[Obsolete("Wrapper types cannot be constructed with constructors (it only instantiate the underlying Pasture3DData object), please use the Instantiate() method instead.")]
	protected Pasture3DData() { }

	private static CSharpScript _wrapperScriptAsset;

	/// <summary>
	/// Try to cast the script on the supplied <paramref name="godotObject"/> to the <see cref="Pasture3DData"/> wrapper type,
	/// if no script has attached to the type, or the script attached to the type does not inherit the <see cref="Pasture3DData"/> wrapper type,
	/// a new instance of the <see cref="Pasture3DData"/> wrapper script will get attaches to the <paramref name="godotObject"/>.
	/// </summary>
	/// <remarks>The developer should only supply the <paramref name="godotObject"/> that represents the correct underlying GDExtension type.</remarks>
	/// <param name="godotObject">The <paramref name="godotObject"/> that represents the correct underlying GDExtension type.</param>
	/// <returns>The existing or a new instance of the <see cref="Pasture3DData"/> wrapper script attached to the supplied <paramref name="godotObject"/>.</returns>
	public new static Pasture3DData Bind(GodotObject godotObject)
	{
		if (!IsInstanceValid(godotObject))
			return null;

		if (godotObject is Pasture3DData wrapperScriptInstance)
			return wrapperScriptInstance;

#if DEBUG
		var expectedType = typeof(Pasture3DData);
		...
		{
			var scriptPathAttribute = typeof(Pasture3DData).GetCustomAttributes<ScriptPathAttribute>().FirstOrDefault();
		...
		return (Pasture3DData)InstanceFromId(instanceId);
	}

	/// <summary>
	/// Creates an instance of the GDExtension <see cref="Pasture3DData"/> type, and attaches a wrapper script instance to it.
	/// </summary>
	/// <returns>The wrapper instance linked to the underlying GDExtension "Pasture3DData" type.</returns>
	public new static Pasture3DData Instantiate() => Bind(ClassDB.Instantiate(NativeName).As<GodotObject>());
	...
	public new Pasture3DRegion GetRegion(Vector2I regionLocation) => 
		Pasture3DRegion.Bind(Call(GDExtensionMethodName.GetRegion, [regionLocation]).As<Resource>());

	public new Pasture3DRegion GetRegionp(Vector3 globalPosition) => 
		Pasture3DRegion.Bind(Call(GDExtensionMethodName.GetRegionp, [globalPosition]).As<Resource>());
	...
	public new Pasture3DRegion AddRegionBlankp(Vector3 globalPosition, bool update = true) => 
		Pasture3DRegion.Bind(Call(GDExtensionMethodName.AddRegionBlankp, [globalPosition, update]).As<Resource>());

	public new Pasture3DRegion AddRegionBlank(Vector2I regionLocation, bool update = true) => 
		Pasture3DRegion.Bind(Call(GDExtensionMethodName.AddRegionBlank, [regionLocation, update]).As<Resource>());

	public new Error AddRegion(Pasture3DRegion region, bool update = true) => 
	...
	public new void RemoveRegion(Pasture3DRegion region, bool update = true) => 
	...
	public new Godot.Collections.Array GetMaps(Pasture3DRegion.MapType mapType) => 
		Call(GDExtensionMethodName.GetMaps, [Variant.From(mapType)]).As<Godot.Collections.Array>();

	public new void UpdateMaps(Pasture3DRegion.MapType mapType = Pasture3DRegion.MapType.Max, bool allRegions = true, bool generateMipmaps = false) => 
	...
	public new void SetPixel(Pasture3DRegion.MapType mapType, Vector3 globalPosition, Color pixel) => 
		Call(GDExtensionMethodName.SetPixel, [Variant.From(mapType), globalPosition, pixel]);

	public new Color GetPixel(Pasture3DRegion.MapType mapType, Vector3 globalPosition) => 
	...
	public new Vector3 GetMeshVertex(long lod, Pasture3DData.HeightFilter filter, Vector3 globalPosition) => 
	...
	public new Error ExportImage(string fileName, Pasture3DRegion.MapType mapType) => 
		Call(GDExtensionMethodName.ExportImage, [fileName, Variant.From(mapType)]).As<Error>();

	public new Image LayeredToImage(Pasture3DRegion.MapType mapType) => 
...
{
public static int SafeAsInt32(this Pasture3DData.HeightFilter enumValue) =>
Convert.ToInt32(enumValue);

public static int SafeAsInt32(this Pasture3DData.HeightFilter enumValue, int defaultValue) =>
Convert.ToInt32(enumValue);

public static int SafeAsInt32(this Pasture3DData.HeightFilter? enumValue, int defaultValue = 0) =>
