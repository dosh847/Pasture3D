...
[Tool]
public partial class Pasture3DTextureAsset : Resource
{

	private new static readonly StringName NativeName = new StringName("Pasture3DTextureAsset");

	[Obsolete("Wrapper types cannot be constructed with constructors (it only instantiate the underlying Pasture3DTextureAsset object), please use the Instantiate() method instead.")]
	protected Pasture3DTextureAsset() { }

	private static CSharpScript _wrapperScriptAsset;

	/// <summary>
	/// Try to cast the script on the supplied <paramref name="godotObject"/> to the <see cref="Pasture3DTextureAsset"/> wrapper type,
	/// if no script has attached to the type, or the script attached to the type does not inherit the <see cref="Pasture3DTextureAsset"/> wrapper type,
	/// a new instance of the <see cref="Pasture3DTextureAsset"/> wrapper script will get attaches to the <paramref name="godotObject"/>.
	/// </summary>
	/// <remarks>The developer should only supply the <paramref name="godotObject"/> that represents the correct underlying GDExtension type.</remarks>
	/// <param name="godotObject">The <paramref name="godotObject"/> that represents the correct underlying GDExtension type.</param>
	/// <returns>The existing or a new instance of the <see cref="Pasture3DTextureAsset"/> wrapper script attached to the supplied <paramref name="godotObject"/>.</returns>
	public new static Pasture3DTextureAsset Bind(GodotObject godotObject)
	{
		if (!IsInstanceValid(godotObject))
			return null;

		if (godotObject is Pasture3DTextureAsset wrapperScriptInstance)
			return wrapperScriptInstance;

#if DEBUG
		var expectedType = typeof(Pasture3DTextureAsset);
		...
		{
			var scriptPathAttribute = typeof(Pasture3DTextureAsset).GetCustomAttributes<ScriptPathAttribute>().FirstOrDefault();
		...
		return (Pasture3DTextureAsset)InstanceFromId(instanceId);
	}

	/// <summary>
	/// Creates an instance of the GDExtension <see cref="Pasture3DTextureAsset"/> type, and attaches a wrapper script instance to it.
	/// </summary>
	/// <returns>The wrapper instance linked to the underlying GDExtension "Pasture3DTextureAsset" type.</returns>
	public new static Pasture3DTextureAsset Instantiate() => Bind(ClassDB.Instantiate(NativeName).As<GodotObject>());
