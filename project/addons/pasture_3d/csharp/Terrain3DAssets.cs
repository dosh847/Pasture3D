...
[Tool]
public partial class Pasture3DAssets : Resource
{

	private new static readonly StringName NativeName = new StringName("Pasture3DAssets");

	[Obsolete("Wrapper types cannot be constructed with constructors (it only instantiate the underlying Pasture3DAssets object), please use the Instantiate() method instead.")]
	protected Pasture3DAssets() { }

	private static CSharpScript _wrapperScriptAsset;

	/// <summary>
	/// Try to cast the script on the supplied <paramref name="godotObject"/> to the <see cref="Pasture3DAssets"/> wrapper type,
	/// if no script has attached to the type, or the script attached to the type does not inherit the <see cref="Pasture3DAssets"/> wrapper type,
	/// a new instance of the <see cref="Pasture3DAssets"/> wrapper script will get attaches to the <paramref name="godotObject"/>.
	/// </summary>
	/// <remarks>The developer should only supply the <paramref name="godotObject"/> that represents the correct underlying GDExtension type.</remarks>
	/// <param name="godotObject">The <paramref name="godotObject"/> that represents the correct underlying GDExtension type.</param>
	/// <returns>The existing or a new instance of the <see cref="Pasture3DAssets"/> wrapper script attached to the supplied <paramref name="godotObject"/>.</returns>
	public new static Pasture3DAssets Bind(GodotObject godotObject)
	{
		if (!IsInstanceValid(godotObject))
			return null;

		if (godotObject is Pasture3DAssets wrapperScriptInstance)
			return wrapperScriptInstance;

#if DEBUG
		var expectedType = typeof(Pasture3DAssets);
		...
		{
			var scriptPathAttribute = typeof(Pasture3DAssets).GetCustomAttributes<ScriptPathAttribute>().FirstOrDefault();
		...
		return (Pasture3DAssets)InstanceFromId(instanceId);
	}

	/// <summary>
	/// Creates an instance of the GDExtension <see cref="Pasture3DAssets"/> type, and attaches a wrapper script instance to it.
	/// </summary>
	/// <returns>The wrapper instance linked to the underlying GDExtension "Pasture3DAssets" type.</returns>
	public new static Pasture3DAssets Instantiate() => Bind(ClassDB.Instantiate(NativeName).As<GodotObject>());
	...
	public new void SetTextureAsset(long id, Pasture3DTextureAsset texture) => 
		Call(GDExtensionMethodName.SetTextureAsset, [id, texture]);

	public new Pasture3DTextureAsset GetTextureAsset(long id) => 
		Pasture3DTextureAsset.Bind(Call(GDExtensionMethodName.GetTextureAsset, [id]).As<Resource>());
	...
	public new void SetMeshAsset(long id, Pasture3DMeshAsset mesh) => 
		Call(GDExtensionMethodName.SetMeshAsset, [id, mesh]);

	public new Pasture3DMeshAsset GetMeshAsset(long id) => 
		Pasture3DMeshAsset.Bind(Call(GDExtensionMethodName.GetMeshAsset, [id]).As<Resource>());
...
{
public static int SafeAsInt32(this Pasture3DAssets.AssetType enumValue) =>
Convert.ToInt32(enumValue);

public static int SafeAsInt32(this Pasture3DAssets.AssetType enumValue, int defaultValue) =>
Convert.ToInt32(enumValue);

public static int SafeAsInt32(this Pasture3DAssets.AssetType? enumValue, int defaultValue = 0) =>
