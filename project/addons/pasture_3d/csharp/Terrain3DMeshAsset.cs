...
[Tool]
public partial class Pasture3DMeshAsset : Resource
{

	private new static readonly StringName NativeName = new StringName("Pasture3DMeshAsset");

	[Obsolete("Wrapper types cannot be constructed with constructors (it only instantiate the underlying Pasture3DMeshAsset object), please use the Instantiate() method instead.")]
	protected Pasture3DMeshAsset() { }

	private static CSharpScript _wrapperScriptAsset;

	/// <summary>
	/// Try to cast the script on the supplied <paramref name="godotObject"/> to the <see cref="Pasture3DMeshAsset"/> wrapper type,
	/// if no script has attached to the type, or the script attached to the type does not inherit the <see cref="Pasture3DMeshAsset"/> wrapper type,
	/// a new instance of the <see cref="Pasture3DMeshAsset"/> wrapper script will get attaches to the <paramref name="godotObject"/>.
	/// </summary>
	/// <remarks>The developer should only supply the <paramref name="godotObject"/> that represents the correct underlying GDExtension type.</remarks>
	/// <param name="godotObject">The <paramref name="godotObject"/> that represents the correct underlying GDExtension type.</param>
	/// <returns>The existing or a new instance of the <see cref="Pasture3DMeshAsset"/> wrapper script attached to the supplied <paramref name="godotObject"/>.</returns>
	public new static Pasture3DMeshAsset Bind(GodotObject godotObject)
	{
		if (!IsInstanceValid(godotObject))
			return null;

		if (godotObject is Pasture3DMeshAsset wrapperScriptInstance)
			return wrapperScriptInstance;

#if DEBUG
		var expectedType = typeof(Pasture3DMeshAsset);
		...
		{
			var scriptPathAttribute = typeof(Pasture3DMeshAsset).GetCustomAttributes<ScriptPathAttribute>().FirstOrDefault();
		...
		return (Pasture3DMeshAsset)InstanceFromId(instanceId);
	}

	/// <summary>
	/// Creates an instance of the GDExtension <see cref="Pasture3DMeshAsset"/> type, and attaches a wrapper script instance to it.
	/// </summary>
	/// <returns>The wrapper instance linked to the underlying GDExtension "Pasture3DMeshAsset" type.</returns>
	public new static Pasture3DMeshAsset Instantiate() => Bind(ClassDB.Instantiate(NativeName).As<GodotObject>());
	...
	public new Pasture3DMeshAsset.GenType GeneratedType
	{
		get => Get(GDExtensionPropertyName.GeneratedType).As<Pasture3DMeshAsset.GenType>();
...
{
public static int SafeAsInt32(this Pasture3DMeshAsset.GenType enumValue) =>
Convert.ToInt32(enumValue);

public static int SafeAsInt32(this Pasture3DMeshAsset.GenType enumValue, int defaultValue) =>
Convert.ToInt32(enumValue);

public static int SafeAsInt32(this Pasture3DMeshAsset.GenType? enumValue, int defaultValue = 0) =>
