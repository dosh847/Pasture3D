...
[Tool]
public partial class Pasture3DCollision : GodotObject
{

	private new static readonly StringName NativeName = new StringName("Pasture3DCollision");

	[Obsolete("Wrapper types cannot be constructed with constructors (it only instantiate the underlying Pasture3DCollision object), please use the Instantiate() method instead.")]
	protected Pasture3DCollision() { }

	private static CSharpScript _wrapperScriptAsset;

	/// <summary>
	/// Try to cast the script on the supplied <paramref name="godotObject"/> to the <see cref="Pasture3DCollision"/> wrapper type,
	/// if no script has attached to the type, or the script attached to the type does not inherit the <see cref="Pasture3DCollision"/> wrapper type,
	/// a new instance of the <see cref="Pasture3DCollision"/> wrapper script will get attaches to the <paramref name="godotObject"/>.
	/// </summary>
	/// <remarks>The developer should only supply the <paramref name="godotObject"/> that represents the correct underlying GDExtension type.</remarks>
	/// <param name="godotObject">The <paramref name="godotObject"/> that represents the correct underlying GDExtension type.</param>
	/// <returns>The existing or a new instance of the <see cref="Pasture3DCollision"/> wrapper script attached to the supplied <paramref name="godotObject"/>.</returns>
	public new static Pasture3DCollision Bind(GodotObject godotObject)
	{
		if (!IsInstanceValid(godotObject))
			return null;

		if (godotObject is Pasture3DCollision wrapperScriptInstance)
			return wrapperScriptInstance;

#if DEBUG
		var expectedType = typeof(Pasture3DCollision);
		...
		{
			var scriptPathAttribute = typeof(Pasture3DCollision).GetCustomAttributes<ScriptPathAttribute>().FirstOrDefault();
		...
		return (Pasture3DCollision)InstanceFromId(instanceId);
	}

	/// <summary>
	/// Creates an instance of the GDExtension <see cref="Pasture3DCollision"/> type, and attaches a wrapper script instance to it.
	/// </summary>
	/// <returns>The wrapper instance linked to the underlying GDExtension "Pasture3DCollision" type.</returns>
	public new static Pasture3DCollision Instantiate() => Bind(ClassDB.Instantiate(NativeName).As<GodotObject>());
	...
	public new Pasture3DCollision.CollisionMode Mode
	{
		get => Get(GDExtensionPropertyName.Mode).As<Pasture3DCollision.CollisionMode>();
...
{
public static int SafeAsInt32(this Pasture3DCollision.CollisionMode enumValue) =>
Convert.ToInt32(enumValue);

public static int SafeAsInt32(this Pasture3DCollision.CollisionMode enumValue, int defaultValue) =>
Convert.ToInt32(enumValue);

public static int SafeAsInt32(this Pasture3DCollision.CollisionMode? enumValue, int defaultValue = 0) =>
