...
[Tool]
public partial class Pasture3DRegion : Resource
{

	private new static readonly StringName NativeName = new StringName("Pasture3DRegion");

	[Obsolete("Wrapper types cannot be constructed with constructors (it only instantiate the underlying Pasture3DRegion object), please use the Instantiate() method instead.")]
	protected Pasture3DRegion() { }

	private static CSharpScript _wrapperScriptAsset;

	/// <summary>
	/// Try to cast the script on the supplied <paramref name="godotObject"/> to the <see cref="Pasture3DRegion"/> wrapper type,
	/// if no script has attached to the type, or the script attached to the type does not inherit the <see cref="Pasture3DRegion"/> wrapper type,
	/// a new instance of the <see cref="Pasture3DRegion"/> wrapper script will get attaches to the <paramref name="godotObject"/>.
	/// </summary>
	/// <remarks>The developer should only supply the <paramref name="godotObject"/> that represents the correct underlying GDExtension type.</remarks>
	/// <param name="godotObject">The <paramref name="godotObject"/> that represents the correct underlying GDExtension type.</param>
	/// <returns>The existing or a new instance of the <see cref="Pasture3DRegion"/> wrapper script attached to the supplied <paramref name="godotObject"/>.</returns>
	public new static Pasture3DRegion Bind(GodotObject godotObject)
	{
		if (!IsInstanceValid(godotObject))
			return null;

		if (godotObject is Pasture3DRegion wrapperScriptInstance)
			return wrapperScriptInstance;

#if DEBUG
		var expectedType = typeof(Pasture3DRegion);
		...
		{
			var scriptPathAttribute = typeof(Pasture3DRegion).GetCustomAttributes<ScriptPathAttribute>().FirstOrDefault();
		...
		return (Pasture3DRegion)InstanceFromId(instanceId);
	}

	/// <summary>
	/// Creates an instance of the GDExtension <see cref="Pasture3DRegion"/> type, and attaches a wrapper script instance to it.
	/// </summary>
	/// <returns>The wrapper instance linked to the underlying GDExtension "Pasture3DRegion" type.</returns>
	public new static Pasture3DRegion Instantiate() => Bind(ClassDB.Instantiate(NativeName).As<GodotObject>());
	...
	public new void SetMap(Pasture3DRegion.MapType mapType, Image map) => 
		Call(GDExtensionMethodName.SetMap, [Variant.From(mapType), map]);

	public new Image GetMap(Pasture3DRegion.MapType mapType) => 
	...
	public new Image SanitizeMap(Pasture3DRegion.MapType mapType, Image map) => 
	...
	public new Pasture3DRegion Duplicate(bool deep = false) => 
		Pasture3DRegion.Bind(Call(GDExtensionMethodName.Duplicate, [deep]).As<Resource>());
...
{
public static int SafeAsInt32(this Pasture3DRegion.MapType enumValue) =>
Convert.ToInt32(enumValue);

public static int SafeAsInt32(this Pasture3DRegion.MapType enumValue, int defaultValue) =>
Convert.ToInt32(enumValue);

public static int SafeAsInt32(this Pasture3DRegion.MapType? enumValue, int defaultValue = 0) =>
