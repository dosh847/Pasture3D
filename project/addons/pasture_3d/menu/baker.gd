...
const BakeLodDialog: PackedScene = preload("res://addons/pasture_3d/menu/bake_lod_dialog.tscn")
...
- Move the Pasture3D node to be a child of the new node,
...
func _bake_mesh() -> void:
	if plugin.terrain.data.get_region_count() == 0:
		push_error("Pasture3D has no active regions to bake")
		return
	var mesh: Mesh = plugin.terrain.bake_mesh(bake_lod_dialog.lod, Pasture3DData.HEIGHT_FILTER_NEAREST)
	if !mesh:
		push_error("Failed to bake mesh from Pasture3D")
		return

	var undo: EditorUndoRedoManager = plugin.get_undo_redo()
	undo.create_action("Pasture3D Bake ArrayMesh")
...
func _bake_occluder() -> void:
	if plugin.terrain.data.get_region_count() == 0:
		push_error("Pasture3D has no active regions to bake")
		return
	var mesh: Mesh = plugin.terrain.bake_mesh(bake_lod_dialog.lod, Pasture3DData.HEIGHT_FILTER_MINIMUM)
	if !mesh:
		push_error("Failed to bake mesh from Pasture3D")
		return
	...
	var undo: EditorUndoRedoManager = plugin.get_undo_redo()
	undo.create_action("Pasture3D Bake Occluder3D")
...
func find_nav_region_terrains(p_nav_region: NavigationRegion3D) -> Array[Pasture3D]:
	var result: Array[Pasture3D] = []
	...
	if source_mode == NavigationMesh.SOURCE_GEOMETRY_ROOT_NODE_CHILDREN:
		result.append_array(p_nav_region.find_children("", "Pasture3D", true, true))
		return result
	...
	for node in group_nodes:
		if node is Pasture3D:
			result.push_back(node)
		if source_mode == NavigationMesh.SOURCE_GEOMETRY_GROUPS_WITH_CHILDREN:
			result.append_array(node.find_children("", "Pasture3D", true, true))
	
	return result


func find_terrain_nav_regions(p_terrain: Pasture3D) -> Array[NavigationRegion3D]:
...
func bake_nav_mesh() -> void:
	if plugin.nav_region:
		...
		print("Pasture3DNavigation: Finished baking 1 NavigationMesh.")
	
	elif plugin.terrain:
		if plugin.terrain.data.get_region_count() == 0:
			push_error("Pasture3D has no active regions to bake")
			return
		# A Pasture3D is selected. There are potentially multiple navmeshes to bake and we need to
		...
		print("Pasture3DNavigation: Finished baking %d NavigationMesh(es)." % nav_regions.size())
...
func _set_up_navigation() -> void:
	...
	if plugin.terrain == EditorInterface.get_edited_scene_root():
		push_error("Pasture3D Navigation setup not possible if Pasture3D node is scene root")
		return
	if plugin.terrain.data.get_region_count() == 0:
		push_error("Pasture3D has no active regions")
		return
	var terrain: Pasture3D = plugin.terrain
	...
	undo_redo.create_action("Pasture3D Set up Navigation")
...
func _do_set_up_navigation(p_nav_region: NavigationRegion3D, p_terrain: Pasture3D) -> void:
...
func _undo_set_up_navigation(p_nav_region: NavigationRegion3D, p_terrain: Pasture3D) -> void:
