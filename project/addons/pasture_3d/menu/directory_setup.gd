...
# Directory Setup for Pasture3D
extends Node

const DIRECTORY_SETUP: String = "res://addons/pasture_3d/menu/directory_setup.tscn"
...
func _on_ok_pressed() -> void:
	if not plugin.terrain:
		push_error("Not connected terrain. Click the Pasture3D node first")
		return
	...
	for file in files:
		if file.begins_with("pasture3d") || file.ends_with(".res"):
			data_found = true
			break
