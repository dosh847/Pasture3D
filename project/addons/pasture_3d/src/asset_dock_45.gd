...
const ES_DOCK_SLOT: String = "pasture3d/dock/slot"
const ES_DOCK_TILE_SIZE: String = "pasture3d/dock/tile_size"
const ES_DOCK_FLOATING: String = "pasture3d/dock/floating"
const ES_DOCK_PINNED: String = "pasture3d/dock/always_on_top"
const ES_DOCK_WINDOW_POSITION: String = "pasture3d/dock/window_position"
const ES_DOCK_WINDOW_SIZE: String = "pasture3d/dock/window_size"
const ES_DOCK_TAB: String = "pasture3d/dock/tab"
...
func _gui_input(p_event: InputEvent) -> void:
	if p_event is InputEventMouseButton:
		if search_box.has_focus():
			if plugin.debug:
				print("Pasture3DAssetDock: _on_box_gui_input: search_box releasing focus")
...
func set_slot(p_slot: int) -> void:
	if plugin.debug:
		print("Pasture3DAssetDock: set_slot: ", p_slot)
...
func update_dock() -> void:
	...
	# Bottom
		...
		state = BOTTOM
		plugin.add_control_to_bottom_panel(self, "Pasture3D")
		plugin.make_bottom_panel_item_visible(self)


func update_layout() -> void:
	if plugin.debug > 1:
		print("Pasture3DAssetDock: update_layout")	
	if not _initialized:
		return
...
func _on_search_text_changed() -> void:
	if plugin.debug:
		print("Pasture3DAssetDock: _on_search_text_changed: ", search_box.text)
...
func _on_search_button_pressed() -> void:
	if plugin.debug:
		print("Pasture3DAssetDock: _on_search_button_pressed")
	if search_box.text.length() > 0:
		search_box.text = ""
		_on_search_text_changed()
	else:
		if plugin.debug:
			print("Pasture3DAssetDock: _on_search_button_pressed: Search box grabbing focus")
...
func _on_textures_pressed() -> void:
	if plugin.debug:
		print("Pasture3DAssetDock: _on_textures_pressed")
...
func _on_meshes_pressed() -> void:
	if plugin.debug:
		print("Pasture3DAssetDock: _on_meshes_pressed")
...
func _on_tool_changed(p_tool: Pasture3DEditor.Tool, p_operation: Pasture3DEditor.Operation) -> void:
	if plugin.debug:
		print("Pasture3DAssetDock: _on_tool_changed: ", p_tool, ", ", p_operation)
	remove_all_highlights()
	if p_tool == Pasture3DEditor.INSTANCER:
		_on_meshes_pressed()
	elif p_tool in [ Pasture3DEditor.TEXTURE, Pasture3DEditor.COLOR, Pasture3DEditor.ROUGHNESS ]:
...
func update_assets() -> void:
	if plugin.debug:
		print("Pasture3DAssetDock: update_assets: ", plugin.terrain.assets if plugin.terrain else "")
	if not _initialized:
		return
...
func remove_all_highlights():
	...
	for i: int in texture_list.entries.size():
		var resource: Pasture3DTextureAsset = texture_list.entries[i].resource
		if resource and resource.is_highlighted():
			resource.set_highlighted(false)
	for i: int in mesh_list.entries.size():
		var resource: Pasture3DMeshAsset = mesh_list.entries[i].resource
		if resource and resource.is_highlighted():
			resource.set_highlighted(false)
...
func make_dock_float() -> void:
	...
	window.title = "Pasture3D Asset Dock"
	window.always_on_top = pinned_btn.button_pressed
...
func _on_godot_window_entered() -> void:
	if plugin.debug > 1:
		print("Pasture3DAssetDock: _on_godot_window_entered")
...
func _on_godot_focus_entered() -> void:
	if plugin.debug > 1:
		print("Pasture3DAssetDock: _on_godot_focus_entered")
...
func _on_godot_focus_exited() -> void:
	if plugin.debug > 1:
		print("Pasture3DAssetDock: _on_godot_focus_exited")
...
class ListContainer extends Container:
	var plugin: EditorPlugin
	var type := Pasture3DAssets.TYPE_TEXTURE
	...
	func update_asset_list() -> void:
		if plugin.debug:
			print("Pasture3DListContainer ", name, ": update_asset_list")
		clear()
		
		# Grab terrain
		var t: Pasture3D = plugin.get_terrain()
		if not (t and t.assets):
			return
		
		if type == Pasture3DAssets.TYPE_TEXTURE:
			var texture_count: int = t.assets.get_texture_count()
			for i in texture_count:
				var texture: Pasture3DTextureAsset = t.assets.get_texture_asset(i)
				add_item(texture)
			if texture_count < Pasture3DAssets.MAX_TEXTURES:
				add_item()
		else:
			if plugin.terrain:
			...
			for i in mesh_count:
				var mesh: Pasture3DMeshAsset = t.assets.get_mesh_asset(i)
				add_item(mesh)
			if mesh_count < Pasture3DAssets.MAX_MESHES:
				add_item()
	...
	func _on_resource_hovered(p_id: int):
		if type == Pasture3DAssets.TYPE_MESH:
			if plugin.terrain:
				plugin.terrain.assets.create_mesh_thumbnails(p_id, Vector2i(512, 512), true)

	
	func set_selected_after_swap(p_type: Pasture3DAssets.AssetType, p_old_id: int, p_new_id: int) -> void:
	...
	func clicked_id(p_id: int) -> void:
		...
		plugin.select_terrain()
		if type == Pasture3DAssets.TYPE_TEXTURE and \
				not plugin.editor.get_tool() in [ Pasture3DEditor.TEXTURE, Pasture3DEditor.COLOR, Pasture3DEditor.ROUGHNESS ]:
			plugin.ui.toolbar.change_tool("PaintTexture")
		elif type == Pasture3DAssets.TYPE_MESH and plugin.editor.get_tool() != Pasture3DEditor.INSTANCER:
	...
	func set_selected_id(p_id: int) -> void:
		...
		if plugin.debug:
			print("Pasture3DListContainer ", name, ": set_selected_id: ", selected_id, " to ", clamp(p_id, 0, max_id))
	...
	func get_selected_asset_id() -> int:
		...
		if plugin.debug:
			print("Pasture3DListContainer ", name, ": get_selected_asset_id: selected_id: ", selected_id, ", clamped: ", id, ", entries: ", entries.size())
		if id >= entries.size():
			return 0
		var res: Resource = entries[id].resource
		if not res:
			return 0
		if type == Pasture3DAssets.AssetType.TYPE_MESH:
			return (res as Pasture3DMeshAsset).id
		else:
			return (res as Pasture3DTextureAsset).id
	...
	func _on_resource_changed(p_resource: Resource, p_id: int) -> void:
		...
		if not p_resource:
			if plugin.debug:
				print("Pasture3DListContainer ", name, ": _on_resource_changed: removing asset ID: ", p_id)
...
class ListEntry extends MarginContainer:
	...
	func setup_buttons() -> void:
		...
		button_highlight.tooltip_text = "Highlight " + ( "Instances" if type == Pasture3DAssets.TYPE_MESH else "Texture" )
	...
	func get_resource_name() -> StringName:
		if resource:
			if resource is Pasture3DMeshAsset:
				return (resource as Pasture3DMeshAsset).get_name()
			elif resource is Pasture3DTextureAsset:
				return (resource as Pasture3DTextureAsset).get_name()
		return ""
	...
	func setup_count_label() -> void:
		...
		var mesh_resource: Pasture3DMeshAsset = resource as Pasture3DMeshAsset
	...
	func update_count_label() -> void:
		if not type == Pasture3DAssets.AssetType.TYPE_MESH or \
	...
	func _notification(p_what) -> void:
		match p_what:
			...
			NOTIFICATION_DRAW:
				...
				spacer.visible = size.x > 94. or type == Pasture3DAssets.TYPE_TEXTURE
			...
			if p_event.is_pressed():
				match p_event.get_button_index():
					MOUSE_BUTTON_LEFT:
						...
						if !resource:
							if type == Pasture3DAssets.TYPE_TEXTURE:
								set_edited_resource(Pasture3DTextureAsset.new(), false)
							else:
								set_edited_resource(Pasture3DMeshAsset.new(), false)
							_on_edit()
						else:
							emit_signal("clicked")
					MOUSE_BUTTON_RIGHT:
	...
	func _drop_data(p_at_position: Vector2, p_data: Variant) -> void:
		if typeof(p_data) == TYPE_DICTIONARY:
			...
			if res is Texture2D and type == Pasture3DAssets.TYPE_TEXTURE:
				var ta := Pasture3DTextureAsset.new()
				if resource is Pasture3DTextureAsset:
					ta.id = resource.id
				ta.set_albedo_texture(res)
				set_edited_resource(ta, false)
				resource = ta
			elif res is Pasture3DTextureAsset and type == Pasture3DAssets.TYPE_TEXTURE:
				if resource is Pasture3DTextureAsset:
					res.id = resource.id
				set_edited_resource(res, false)
			elif res is PackedScene and type == Pasture3DAssets.TYPE_MESH:
				if not resource:
					resource = Pasture3DMeshAsset.new()		
				set_edited_resource(resource, false)
				resource.set_scene_file(res)
			elif res is Pasture3DMeshAsset and type == Pasture3DAssets.TYPE_MESH:
				if resource is Pasture3DMeshAsset:
					res.id = resource.id
				set_edited_resource(res, false)
			emit_signal("clicked")
	...
	func set_edited_resource(p_res: Resource, p_no_signal: bool = true) -> void:
		...
		if resource:
			...
			elif resource is Pasture3DMeshAsset:
