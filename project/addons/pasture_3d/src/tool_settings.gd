...
# Tool settings bar for Pasture3D
...
const MultiPicker: Script = preload("res://addons/pasture_3d/src/multi_picker.gd")
const DEFAULT_BRUSH: String = "circle0.exr"
const BRUSH_PATH: String = "res://addons/pasture_3d/brushes"
const ES_TOOL_SETTINGS: String = "pasture3d/tool_settings/"
...
func _ready() -> void:
	...
	add_setting({ "name":"height_picker", "type":SettingType.PICKER, "list":main_list, "default":Pasture3DEditor.HEIGHT,
	...
	add_setting({ "name":"color_picker", "type":SettingType.PICKER, "list":main_list, "default":Pasture3DEditor.COLOR,
	...
	add_setting({ "name":"roughness_picker", "type":SettingType.PICKER, "list":main_list, "default":Pasture3DEditor.ROUGHNESS,
	...
	add_setting({ "name":"texture_picker", "type":SettingType.PICKER, "list":main_list, "default":Pasture3DEditor.TEXTURE,
	...
	add_setting({ "name":"angle_picker", "type":SettingType.PICKER, "list":main_list, "default":Pasture3DEditor.ANGLE,
	...
	add_setting({ "name":"scale_picker", "type":SettingType.PICKER, "list":main_list, "default":Pasture3DEditor.SCALE,
	...
	add_setting({ "name":"gradient_points", "type":SettingType.MULTI_PICKER, "label":"Points", 
							"list":main_list, "default":Pasture3DEditor.SCULPT, "flags":ADD_SEPARATOR })
	...
	add_setting({ "name":"mesh_picker", "type":SettingType.PICKER, "list":main_list,
							"default":Pasture3DEditor.INSTANCER, "flags":NO_LABEL|ADD_SEPARATOR,
...
func create_submenu(p_parent: Control, p_button_name: String, p_layout: Layout, p_hover_pop: bool = true) -> Container:
	...
	submenu.add_to_group("pasture3d_submenus")
...
func _on_show_submenu(p_toggled: bool, p_button: Button) -> void:
	...
	get_tree().call_group("pasture3d_submenus", "set_visible", false)
	...
	if dir:
		...
		while file_name != "":
			if !dir.current_is_dir() and file_name.ends_with(".exr"):
				...
				if img:
					var value_range: Vector2 = Pasture3DUtil.get_min_max(img)
					...
					thumbimg = Pasture3DUtil.black_to_alpha(thumbimg)
...
func _on_pick(p_type: Pasture3DEditor.Tool) -> void:
	if plugin.debug:
		print("Pasture3DToolSettings: _on_pick: emitting picking: ", p_type, ", ", _on_picked)
	emit_signal("picking", p_type, _on_picked)


func _on_picked(p_type: Pasture3DEditor.Tool, p_color: Color, p_global_position: Vector3) -> void:
	match p_type:
...
func _on_point_pick(p_type: Pasture3DEditor.Tool, p_name: String) -> void:
	assert(p_type == Pasture3DEditor.SCULPT)
	if plugin.debug:
		print("Pasture3DToolSettings: _on_pick: emitting picking: ", p_type, ", ", _on_point_picked)
	emit_signal("picking", p_type, _on_point_picked.bind(p_name))


func _on_point_picked(p_type: Pasture3DEditor.Tool, p_color: Color, p_global_position: Vector3, p_name: String) -> void:
	assert(p_type == Pasture3DEditor.SCULPT)
...
func _on_setting_changed(p_setting: Variant = null) -> void:
	...
	if plugin.debug:
		print("Pasture3DToolSettings: _on_setting_changed: emitting setting_changed: ", p_setting)			
