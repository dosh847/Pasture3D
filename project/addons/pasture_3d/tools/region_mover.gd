...
# 
# Attach it to your Pasture3D node
# Save and reload your scene
# Select your Pasture3D node
...
func start_rename() -> void:
	...
	for file_name in files:
		if file_name.match("pasture3d*.res") and not dir.current_is_dir():
			var region_loc: Vector2i = Pasture3DUtil.filename_to_location(file_name)
			...
			if new_loc.x < -16 or new_loc.x > 15 or new_loc.y < -16 or new_loc.y > 15:
				...
				return
			var new_name: String = "tmp_" + Pasture3DUtil.location_to_filename(new_loc)
