...
func reset_terrain() -> void:
	data_directory = ""
	for region:Pasture3DRegion in data.get_regions_active():
		data.remove_region(region, false)
	data.update_maps(Pasture3DRegion.TYPE_MAX, true, false)
...
func start_import() -> void:
	print("Pasture3DImporter: Importing files:\n\t%s\n\t%s\n\t%s" % [ height_file_name, control_file_name, color_file_name])

	var imported_images: Array[Image]
	imported_images.resize(Pasture3DRegion.TYPE_MAX)
	var min_max := Vector2(0, 1)
	var img: Image
	if height_file_name:
		img = Pasture3DUtil.load_image(height_file_name, ResourceLoader.CACHE_MODE_IGNORE, r16_range, r16_size)
		min_max = Pasture3DUtil.get_min_max(img)
		imported_images[Pasture3DRegion.TYPE_HEIGHT] = img
	if control_file_name:
		img = Pasture3DUtil.load_image(control_file_name, ResourceLoader.CACHE_MODE_IGNORE)
		imported_images[Pasture3DRegion.TYPE_CONTROL] = img
	if color_file_name:
		img = Pasture3DUtil.load_image(color_file_name, ResourceLoader.CACHE_MODE_IGNORE)
		imported_images[Pasture3DRegion.TYPE_COLOR] = img
	...
	print("Pasture3DImporter: Import finished")
...
func start_export() -> void:
	...
	print("Pasture3DImporter: Export error status: ", err, " ", error_string(err))
