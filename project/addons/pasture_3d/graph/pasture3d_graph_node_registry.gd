...
const PathCarveScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_path_carve.gd")
...
const SplineSourceScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_spline_source.gd")
const PathWidthScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_path_width.gd")
const PathResampleScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_path_resample.gd")
const PathSmoothScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_path_smooth.gd")
const PathDecimateScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_path_decimate.gd")
const PathDrapeScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_path_drape.gd")
const PathWidthFieldScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_path_width_field.gd")
const PathFromFlowScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_path_from_flow.gd")
const PathFractalizeScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_path_fractalize.gd")
const PathMeanderizeScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_path_meanderize.gd")
...
const DevPathCarveScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_path_carve.gd")
const DevLevelerScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_leveler.gd")
const LevelerScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_leveler.gd")
...
const GradientScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_gradient.gd")
const DevGradientScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_gradient.gd")
const ValueRampScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_value_ramp.gd")
const DevValueRampScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_value_ramp.gd")
const ColorRampScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_color_ramp.gd")
const DevColorRampScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_color_ramp.gd")
const DevColorBlendScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_color_blend.gd")
const FloatToMaskScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_float_to_mask.gd")
const DevFloatToMaskScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_float_to_mask.gd")
...
const DevPathDrapeScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_path_drape.gd")
const DevPathWidthFieldScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_path_width_field.gd")
const DevPathFromFlowScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_path_from_flow.gd")
const DevPathResampleScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_path_resample.gd")
const DevPathSmoothScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_path_smooth.gd")
const DevPathDecimateScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_path_decimate.gd")
const DevPathFractalizeScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_path_fractalize.gd")
const DevPathMeanderizeScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_path_meanderize.gd")
const DevPathWidthScript = preload("res://addons/pasture_3d/graph/pasture3d_graph_node_dev_path_width.gd")
...
static func categories(p_include_dev: bool = false) -> Array[String]:
	var ordered: Array[String] = [
		...
		"Filters & Modifiers",
		"Solvers & Realism",
		# Paths before Roads: a road is one thing you can make a PATH out of, and the general case reads
		# first. Splitting them is what S1 deferred (§5.4) -- "Roads" was already the wrong name for a
		# family carrying rivers and cliff lines, and by S6 it was thirteen entries deep, eleven of which
		# had nothing to do with roads. Roads keeps the two nodes that really are about roads: the one
		# that names a road in the scene, and the one that grades a carriageway.
		"Paths",
		"Roads",
		"Math & Combiners",
		"Constants",
...
static func entries(p_include_dev: bool = false) -> Array[Dictionary]:
	var list: Array[Dictionary] = [
		...
		{"op": &"const", "title": "Const Float", "category": "Constants", "role": "Constant", "script": ConstScript, "tags": ["constant", "flat", "height", "value", "bias", "float"], "description": "A scalar float constant. Drives any value port, and fills every cell so it also reads as a flat field."},
		...
		{"op": &"gradient", "title": "Gradient", "category": "Generators", "role": "Generator", "script": GradientScript, "tags": ["gradient", "linear", "radial", "spherical", "angular", "diamond", "ramp", "slope", "tilt", "distance", "mask", "brush", "hesiod"], "description": "A metric gradient between two points, in metres relative to the host brush or the world, as a [0,1] mask or a height ramp."},
		{"op": &"value_ramp", "title": "Value Ramp", "category": "Filters & Modifiers", "role": "Filter", "script": ValueRampScript, "tags": ["ramp", "gradient", "colour", "color", "band", "remap", "lookup", "stops", "terrace", "mask", "height", "hesiod"], "description": "Maps the input through a Gradient's stops, evaluated exactly, and reads one channel back as a mask or a height."},
		...
		{"op": &"float_to_mask", "title": "Float to Mask", "category": "Masks", "role": "Filter", "script": FloatToMaskScript, "tags": ["mask", "normalize", "normalise", "remap", "range", "percentile", "fit", "metres", "erosion", "sediment"], "description": "A field in its own units (metres of erosion, deposition, height) to a 0..1 mask, over a fixed window or the field's own percentiles, with invert, gamma, smoothstep and blur."},
		...
		{"op": &"color_ramp", "title": "Color Ramp", "category": "Math & Combiners", "role": "Combiner", "script": ColorRampScript, "tags": ["color", "colour", "ramp", "gradient", "tint", "rgba", "stops", "height", "paint"], "description": "Maps a scalar field through a Gradient's stops to a Color PER CELL, for a Color Sink. Evaluated exactly, natively."},
		...
		{"op": &"shape_source", "title": "Shape Source", "category": "Paths", "role": "Generator", "script": ShapeSourceScript, "tags": ["shape", "outline", "brush", "region", "closed", "ring", "mound", "plow", "pond", "area", "footprint"], "description": "Puts a brush's own outline into the graph as a closed PATH, so a region is drawn once and masked from the same spline."},
		{"op": &"spline_source", "title": "Spline Source", "category": "Paths", "role": "Generator", "script": SplineSourceScript, "tags": ["spline", "path", "curve", "river", "ridge", "canyon", "line", "route", "authored", "draw", "geometry", "crest", "bed"], "description": "Puts a Pasture3DSpline you drew into the graph as a PATH, with a half-width at every vertex and the elevation you authored."},
		{"op": &"path_distance", "title": "Path Distance", "category": "Paths", "role": "Filter", "script": PathDistanceScript, "tags": ["road", "path", "distance", "corridor", "arc", "offset", "falloff", "verge"], "description": "Analytic distance to a PATH, plus arc length s and the signed across-position t. Exact, not jump-flooded."},
		{"op": &"path_mask", "title": "Path Mask", "category": "Paths", "role": "Filter", "script": PathMaskScript, "tags": ["road", "path", "mask", "corridor", "region", "outline", "closed", "feather", "invert"], "description": "A PATH as a [0,1] mask: a corridor along an open path, or the filled interior of a closed one."},
		{"op": &"path_width", "title": "Path Width", "category": "Paths", "role": "Filter", "script": PathWidthScript, "tags": ["path", "spline", "width", "taper", "wide", "narrow", "scale", "half-width", "river", "channel", "corridor"], "description": "Rewrites a path's half-widths -- set them outright or scale what was drawn, optionally tapered along the line -- without moving the line itself."},
		{"op": &"path_resample", "title": "Path Resample", "category": "Paths", "role": "Filter", "script": PathResampleScript, "tags": ["path", "spline", "resample", "subdivide", "densify", "step", "vertices", "curve", "smooth", "catmull", "bezier"], "description": "Puts a vertex every few metres along a path -- straight or through a curve -- so a six-point drawn line has geometry for the rest of the family to work on."},
		{"op": &"path_smooth", "title": "Path Smooth", "category": "Paths", "role": "Filter", "script": PathSmoothScript, "tags": ["path", "spline", "smooth", "average", "relax", "denoise", "wobble", "hand-drawn", "river", "ridge"], "description": "Takes the hand out of a hand-drawn line with a moving average, with a one-sided lag that kills wobble without flattening genuine bends."},
		{"op": &"path_decimate", "title": "Path Decimate", "category": "Paths", "role": "Filter", "script": PathDecimateScript, "tags": ["path", "spline", "decimate", "simplify", "reduce", "visvalingam", "cost", "vertices", "optimise"], "description": "Removes the vertices that contribute least area (Visvalingam-Whyatt), keeping the shape of a curve rather than only its extremes."},
		{"op": &"path_fractalize", "title": "Path Fractalize", "category": "Paths", "role": "Filter", "script": PathFractalizeScript, "tags": ["path", "spline", "fractal", "fractalize", "rough", "noise", "midpoint", "displacement", "coastline", "cliff", "seed"], "description": "Midpoint-displacement roughness along a line, in metres -- a coastline or a cliff edge that should not read as drawn."},
		{"op": &"path_meanderize", "title": "Path Meanderize", "category": "Paths", "role": "Filter", "script": PathMeanderizeScript, "tags": ["path", "spline", "meander", "meanderize", "river", "wind", "sinuous", "bend", "channel", "loop", "seed"], "description": "Amplifies a line's own curvature the way water does, turning a drawn river into a meandering one, and cuts out the loops that produces."},
		{"op": &"path_drape", "title": "Path Drape", "category": "Paths", "role": "Filter", "script": PathDrapeScript, "tags": ["path", "spline", "drape", "height", "surface", "sit", "conform", "downhill", "river", "flow", "bed", "terrain"], "description": "Gives a path the heights of the terrain under it, optionally clamped so it never runs uphill -- a river bed that follows ground erosion has not finished making."},
		{"op": &"path_width_field", "title": "Path Width from Field", "category": "Paths", "role": "Filter", "script": PathWidthFieldScript, "tags": ["path", "spline", "width", "field", "flow", "accumulation", "erosion", "downstream", "widen", "river", "catchment"], "description": "Reads a path's half-widths off a field -- flow accumulation, in practice -- so a river is narrow at its head and wide at its mouth because the water says so."},
		{"op": &"path_from_flow", "title": "Path from Flow", "category": "Paths", "role": "Generator", "script": PathFromFlowScript, "tags": ["path", "river", "flow", "accumulation", "trace", "stream", "channel", "watercourse", "erosion", "main stem", "outlet", "seed"], "description": "Traces the main stem of a river out of a flow-accumulation field: no spline drawn, the line comes from where the water actually goes."},
		{"op": &"leveler", "title": "Leveler", "category": "Filters & Modifiers", "role": "Filter", "script": LevelerScript, "tags": ["level", "flatten", "pad", "platform", "plateau", "building", "mean", "median", "cut", "fill", "loop", "area", "walls"], "description": "Flattens an area to its mean, median, min or max, or levels it to a height, with a feathered wall outside. The area is a closed loop, a mask, or the brush footprint."},
		{"op": &"path_carve", "title": "Path Carve", "category": "Paths", "role": "Solver", "script": PathCarveScript, "tags": ["path", "spline", "carve", "ridge", "trough", "crest", "bed", "river", "canyon", "channel", "levee", "flank", "cross-section"], "description": "Cuts one cross-section along a path -- a raised crest or a carved bed, with a flat top, a flank and the bed, flank, cut and fill masks."},
...
static func _dev_entries() -> Array[Dictionary]:
	return [
		...
		{"op": &"dev_gradient", "title": "[Dev/GD] Gradient", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevGradientScript, "tags": ["dev", "gdscript", "oracle", "gradient", "linear", "radial", "spherical", "angular", "ramp", "distance"], "description": "The GDScript definition the Gradient's C++ and GPU kernels are measured against."},
		{"op": &"dev_color_ramp", "title": "[Dev/GD] Color Ramp", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevColorRampScript, "tags": ["dev", "gdscript", "oracle", "ramp", "gradient", "color", "colour"], "description": "The Color Ramp mapped through Gradient.sample per cell, the reference color_ramp_cells is measured against."},
		{"op": &"dev_color_blend", "title": "[Dev/GD] Color Blend", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevColorBlendScript, "tags": ["dev", "gdscript", "oracle", "blend", "color", "colour"], "description": "The Color Blend folded per cell in GDScript, the reference color_blend_cells is measured against."},
		{"op": &"dev_float_to_mask", "title": "[Dev/GD] Float to Mask", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevFloatToMaskScript, "tags": ["dev", "gdscript", "oracle", "mask", "normalize"], "description": "Float to Mask in GDScript, the reference float_to_mask_grid is measured against."},
		{"op": &"dev_value_ramp", "title": "[Dev/GD] Value Ramp", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevValueRampScript, "tags": ["dev", "gdscript", "oracle", "ramp", "gradient", "stops", "band"], "description": "The Value Ramp through Gradient.sample, the reference its C++ and GPU kernels are measured against."},
		{"op": &"dev_leveler", "title": "[Dev/GD] Leveler", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevLevelerScript, "tags": ["dev", "gdscript", "oracle", "level", "flatten", "pad", "platform", "plateau", "mean", "median", "cut", "fill", "loop"], "description": "The GDScript definition the Leveler's C++ kernel is measured against: flatten an area to its mean/median/min/max, or level it to a height, with a feathered wall outside."},
		{"op": &"dev_path_carve", "title": "[Dev/GD] Path Carve", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevPathCarveScript, "tags": ["dev", "gdscript", "oracle", "path", "carve", "ridge", "trough", "crest", "bed", "flank", "cross-section"], "description": "The GDScript definition Path Carve's C++ kernel is measured against -- the same two-reference cross-section, cell by cell."},
		{"op": &"dev_path_drape", "title": "[Dev/GD] Path Drape", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevPathDrapeScript, "tags": ["dev", "gdscript", "oracle", "path", "drape", "height", "surface", "terrain", "downhill"], "description": "Pure GDScript reference oracle for Path Drape elevation sampling."},
		{"op": &"dev_path_width_field", "title": "[Dev/GD] Path Width from Field", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevPathWidthFieldScript, "tags": ["dev", "gdscript", "oracle", "path", "width", "field", "flow", "taper"], "description": "Pure GDScript reference oracle for Path Width from Field remapping."},
		{"op": &"dev_path_from_flow", "title": "[Dev/GD] Path from Flow", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevPathFromFlowScript, "tags": ["dev", "gdscript", "oracle", "path", "flow", "river", "stream", "thalweg"], "description": "Pure GDScript reference oracle for Path from Flow streamline tracing."},
		{"op": &"dev_path_resample", "title": "[Dev/GD] Path Resample", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevPathResampleScript, "tags": ["dev", "gdscript", "oracle", "path", "resample", "step", "bezier", "catmull", "cubic"], "description": "Pure GDScript reference oracle for Path Resample arc-length spacing."},
		{"op": &"dev_path_smooth", "title": "[Dev/GD] Path Smooth", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevPathSmoothScript, "tags": ["dev", "gdscript", "oracle", "path", "smooth", "window", "inertia", "average"], "description": "Pure GDScript reference oracle for Path Smooth moving average."},
		{"op": &"dev_path_decimate", "title": "[Dev/GD] Path Decimate", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevPathDecimateScript, "tags": ["dev", "gdscript", "oracle", "path", "decimate", "simplify", "visvalingam"], "description": "Pure GDScript reference oracle for Path Decimate Visvalingam-Whyatt simplification."},
		{"op": &"dev_path_fractalize", "title": "[Dev/GD] Path Fractalize", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevPathFractalizeScript, "tags": ["dev", "gdscript", "oracle", "path", "fractalize", "midpoint", "noise", "displacement"], "description": "Pure GDScript reference oracle for Path Fractalize midpoint noise displacement."},
		{"op": &"dev_path_meanderize", "title": "[Dev/GD] Path Meanderize", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevPathMeanderizeScript, "tags": ["dev", "gdscript", "oracle", "path", "meanderize", "river", "bend", "loop"], "description": "Pure GDScript reference oracle for Path Meanderize curvature amplification."},
		{"op": &"dev_path_width", "title": "[Dev/GD] Path Width", "category": "Dev / Reference", "role": "Dev / Reference", "script": DevPathWidthScript, "tags": ["dev", "gdscript", "oracle", "path", "width", "scale", "set", "taper"], "description": "Pure GDScript reference oracle for Path Width assignment."},
