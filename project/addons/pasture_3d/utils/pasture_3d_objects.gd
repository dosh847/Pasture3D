...
# Objects parent for Pasture3D
# Children nodes get transform updates on sculpting
@tool
extends Node3D
class_name Pasture3DObjects

const TransformChangedNotifier: Script = preload("res://addons/pasture_3d/utils/transform_changed_notifier.gd")
...
func get_terrain() -> Pasture3D:
	var terrain := instance_from_id(_terrain_id) as Pasture3D
	if not terrain or terrain.is_queued_for_deletion() or not terrain.is_inside_tree():
		var terrains: Array[Node] = Engine.get_singleton(&"EditorInterface").get_edited_scene_root().find_children("", "Pasture3D")
		if terrains.size() > 0:
			terrain = terrains[0]
...
func _get_terrain_height(p_global_position: Vector3) -> float:
	var terrain: Pasture3D = get_terrain()
