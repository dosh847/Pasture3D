...
# It provides a `Project on Pasture3D` modifier, which allows Scatter 
# to detect the terrain height from Pasture3D without using collision.
#
# Copy this file into /addons/proton_scatter/src/modifiers
# Then uncomment everything below (select, press CTRL+K)
# In the editor, add this modifier to Scatter, then set your Pasture3D node
...
#
#func _init() -> void:
	#display_name = "Project On Pasture3D"
	#category = "Edit"
...
#
	#documentation.add_paragraph(
		#"This is a modified version of `Project on Colliders` that queries Pasture3D
		#for heights without using collision. It constrains placement by slope or texture.
#
		#This modifier must have terrain_node set to a Pasture3D node.")
#
	#var p := documentation.add_parameter("Terrain Node")
	#p.set_type("NodePath")
	#p.set_description("Set your Pasture3D node.")
		#
...
#
	#if not _terrain:
		#warning += """No Pasture3D node found"""
		#return
#
	#if not _terrain.data:
		#warning += """Pasture3DData is not initialized"""
		#return
#
	## Review transforms
