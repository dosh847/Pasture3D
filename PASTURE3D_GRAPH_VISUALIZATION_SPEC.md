...
**Status:** **V1–V7a BUILT (2026-09-06/07); V7c BUILT (2026-09-07); V7b specified, unbuilt.** V1 ships as `GraphPreviewReprGate` at 114 checks — criteria [A], [A2], [B], [C], [D], [E], [F], [G], [H], [I], each red-watched. One deviation from the table below: [A] is worded inverted relative to §5.2's prose ("renders identically" where the prose requires the two mask fields to render DIFFERENTLY against a shared absolute scale, which is the point of an absolute scale); it is built to the prose, and the gate asserts both halves. One thing found and not fixed, out of V1's scope: `compile_graph_program_multi` ignores `blocks_native()`, so a FROZEN solver previews through a live re-solve. V2 ships as `GraphPreviewChannelGate` at 25 checks, both halves red-watched. One deviation from §8: it names `compile_graph_program_multi` as the allocation half, but that function already emits `out_count` from `native_out_count()` unconditionally — what is demand-driven is the EVALUATOR's liveness pass in `graph_eval_grid_core`. Both halves are in `pasture_3d_graph_ops.cpp` as §8 says, just not in the function it names. The tap return gains a third key, `reserved`, so a gate can assert on the reservation itself rather than on a field being non-zero. V3 ships as `GraphPathOverlayGate` at 34 checks — criteria [A]–[F], built as a plain `RefCounted` (`graph_path_overlay.gd`) so the gate measures the vertex arrays rather than that the gizmo was called. [A]'s control is the criterion and fires: substituting `derive_without_grid` for `derived_path()` fails both halves. `_returned`/`derived_path()` were lifted to `Pasture3DGraphNode` and `_resolved_path_of` made a recording wrapper — `eval_path` cannot record a PATH SOURCE (short-circuited) nor a memo HIT. One documented behaviour turned out not to be load-bearing: the `has_heights` guard on the drop lines is redundant, because a heightless path is drawn AT `get_height` and `DROP_EPSILON` already suppresses the degenerate drops; [C3] pins it with a flat-height control. V4 ships as `GraphInspectGate` at 43 checks — criteria [A], [A2], [A3], [B], [C], [D], [E], [F], all six red watches fired. It carries the channel selector §7 assigned to it on 2026-09-07, so V2's mechanism is reachable by a person for the first time. The measurements are `static func`s over plain arrays (`graph_inspector.gd`), which is what lets the probe be checked against a field tapped OUTSIDE the inspector's own path. V5 ships as `GraphChannelSinkGate` at 50 checks — criteria [A], [B], [B2], [C], [D], [D2], [E], [F], seven red watches fired. The §9.1a prerequisite landed first, in `pasture_3d_editor.cpp`: `route_to_layer` now tests *the active layer is of the map type this TOOL writes* rather than *the map type is HEIGHT*, and `start_operation` captures a control or colour layer the same way it captured a height one — so `is_reserved()` finally refuses a stroke on a sink's layer. Reverting that one change fails [D] five times over, and the failure is the one §9.1a predicted from a code reading rather than a crash: the stroke goes straight into the region map, and the touch-up it lands is erased by the next re-bake. **One deviation from §9.1a's scope:** the ROUGHNESS tool is deliberately NOT routed. A colour overlay's alpha *is* its coverage weight and roughness lives in the colour Base (`_composite_color_region` leaves A untouched), so there is no per-overlay place to put a roughness value; routing it would have dropped the number silently into a weight. It keeps writing the region map, as it does today. **One pre-existing limit found and not fixed:** a control layer stores the packed word as float BITS, so the single word `enc_base(31) | enc_overlay(31)` — 0xFFC00000, a quiet NaN — cannot survive the typed layer at all. It is the layer storage's, not the sink's, and it is recorded in [C]'s comment rather than worked around. `GraphAllNodeSocketsGate` gained a third exemption from its op_ids() sweep, for TERMINAL nodes: a node nothing can wire from is never visited by any compiler pass, so registering its tag would be the actual mistake. The exemption is red-watched — make a sink non-terminal and the sweep catches all four immediately. V6's [A2] can now rely on it. V6 ships as `GraphExportGate` at 79 checks — criteria [A], [A2], [B], [C], [D], [E], plus two the phase table does not list ([F] and [G], below); eight red watches fired. All five sinks are terminal, so [A]'s op-count and output-field equality is a consequence of the mechanism rather than of care, and its control — the same subtree wired into a NON-terminal node — does move the op count. [A2] holds and the terminal-node exemption V5 added does its job: making one sink non-terminal fails ten checks here AND makes `GraphAllNodeSocketsGate` name all five as ops with no kernel behind them. **One thing the spec did not anticipate:** Godot's `Image.save_png()` is 8-bit only. Measured on 4.7, a ramp saved from FORMAT_RGBAH, FORMAT_RGBAF, FORMAT_RF and FORMAT_RGBA8 all came back at ~1/255 and all four reloaded as FORMAT_RGBA8 — so png16 through that door would have been png8 wearing a different format enum, and [C] would have measured nothing. png16 is therefore written by our own minimal encoder (`addons/pasture_3d/src/graph_png.gd`: single IDAT, filter 0, colour type 0 or 6), and ONE writer serves both depths so [C]'s control differs from its subject by one integer. Measured: png16 0.0000076, png8 0.001961 — a ratio of 257, which is what eight fewer bits predicts. **Two criteria added beyond the table,** both for §9.2 rules that were otherwise untested: **[F]** a wired `normal` port is refused by name rather than deriving from `height` and presenting it as the author's vector field (the 2026-09-06 Q3 ruling), with an unwired control that does export; **[G]** an index map is written UNSCALED — all 4096 indices come back as themselves, against a control where the same field through a quantised sink leaves only 1 of 4096 intact. `Export All` is the begin/step/finish shape of `bake_all_brushes()`, so cancellation is *stopping between steps* and needs no rollback: the graph was never touched. The editor gains one **Export All** button, shown only on a graph that has sinks — and no auto-export toggle anywhere, because §12.7's terminality leaves nothing for a flag to suppress. Ten open questions answered 2026-09-06 — eight in §14.1/§14.1b, plus §14.2 items 1 (measured) and 3 (settled). Two remain open (§14.2 items 2 and 4). V7a ships as `GraphRuntimeSinkGate` at 39 checks — criteria [A]–[F] plus a [G] for terminality; five red watches fired. `Path Publish` fills a `Pasture3DRoadRun` on the network’s `Pasture3DRoadRuntime`, keyed on the node’s label so a republish REPLACES rather than appends; `Water Surface Publish` reads `closed` and routes — a loop to a `Pasture3DPool`’s `curve`, an open course to a `Pasture3DStream.published_path` — and refuses the mismatch BY NAME in both directions rather than coercing it. Both sinks are terminal, so V7a adds no op and `graph_op_ids()` is unchanged. **Two defects the gate caught rather than the design predicting them:** the digest was being stamped on the `Pasture3DRoadNetwork`, but `locate()` lives on the `Pasture3DRoadRuntime` it holds, so the flag sat on a node nobody queries and every road consumer read stale forever — hence `Pasture3DGraphNodeRuntimeSink.digest_target()`, which is the consumer itself for a water body and the runtime for a network; and the writer resolved its path WITHOUT the caller’s surface while the staleness check resolved it WITH one, so the publish stamped an un-draped path and the very next check disagreed with it. **One thing the spec did not anticipate:** a `Path Drape` captures its surface during `eval_grid`, so `resolved_path_of` alone re-derives from the PREVIOUS bake’s grid — which would have published a river draped over the ground as it was before the edit that triggered the publish, and would have made the staleness check agree with it. `Pasture3DTerrainGraph.stage_paths_for()` is the public door added for that, and the bake hook passes the same tap domain the B1 channel sinks get. **One criterion rebuilt after it measured nothing:** [G]’s program-identity check passes with `has_output()` flipped to true, because a sink nothing wires FROM changes no program either way — terminality is now asserted directly, with an ordinary filter as its control. `_apply_graph_step` runs the publish ABOVE the frozen/deferred/miss split, exactly as V5’s channel sinks are, so a graph set to Frozen keeps republishing. The consumer dropdown is filled by `Pasture3DGraphSources.resolve_publish_targets`, so the names offered and the names that resolve are one list. **V7c (2026-09-07) — the sink pass reaches the bake on BOTH raster routes, and `layer_key`.** Reported from the editor as “the sink nodes are inert” and reproduced: `_run_graph_sinks` (V5) and `_run_graph_runtime_sinks` (V7a) both lived inside `Pasture3DTerrainBrush._apply_graph_step`, which is the GDScript rasteriser ONLY. A graph whose every op the native evaluator implements takes `BrushModStep::GRAPH` in `pasture_3d_brush_raster.cpp` instead, so on every graph that lowers cleanly — which is every graph worth having — the sinks wrote nothing, cleared nothing, published nothing and said nothing about it. Measured on the repro: `clear_count=0 write_count=0` natively against `clear_count=1` with `force_gdscript_raster`. The fix is NOT to force a graph with sinks onto GDScript, which would cost the brush its native path and is exactly what terminality exists to avoid: `brush_mod_graph` now hands its input surface back through the step's `out` slot (`sink_input`, `sink_gw`, `sink_gh`, `sink_rect`) ABOVE the frozen/deferred/miss split, `_commit_modifier_caches` adopts it onto the modifier, and the pass runs once from `_run_stack_graph_sinks` — the one point BOTH brushes reach on BOTH branches. **A second defect the first was hiding:** `Pasture3DGraphSources.resolve()` was called only from `_apply_graph_step` too, so on the native route nothing resolved a Road Source, a Shape Source or a publish sink's consumer key at bake at all; it is now called from `_compile_modifiers`, before either rasteriser. `layer_key` on `Pasture3DGraphNodeChannelSink` is the §9.1 answer to “assign the layer to the node”: empty (the default) keys the layer on the node INDEX exactly as before, and a shared key makes two sinks author into ONE layer, later over earlier — which required the §8.1 step-2 clear to move from once per SINK to once per LAYER per bake, or the second sink would wipe the first every bake. `Pasture3DData::layer_add_typed` and a type menu on the Layers dock's `+` fix the other half the report named: the dock called `layer_add`, which has no map-type parameter, so it could produce nothing but HEIGHT layers. It is deliberately NOT `create_owned_layer_typed` — that one reserves and owns, so a “control layer I made to paint on” would refuse every stroke and be cleared by the next bake. Ships as `GraphSinkBakeGate` at 29 checks, criteria [A]–[E]; the [A] red watch fails 16 of them. **One mechanism written and removed again:** a scratch grid of authored words, to carry forward an earlier sink's word on a shared layer. Its red watch PASSED with the grid cut out — step 4 recomposites per SINK, not at the end of the run, so `get_control` already saw the earlier write and there was nothing to fix. **One pre-existing defect found and NOT fixed:** the two mound rasterisers disagree about the footprint edge. On [B]'s fixture, same 86×86 grid, same rect and same height range, the native raster leaves 1062 cells NaN and the GDScript raster 938 — 124 cells. That predates the sink pass and belongs to whichever gate owns `stamp_mound_loop` against its oracle; [B] bounds its parity by the loop rim and asserts the interior exactly, and the [A] red watch produces a difference of 6458 against a rim of 344, so the tolerance cannot hide a real break. The substrate V1 and V2 extend — editor-owned
