...
**What S5 built differently.** Seven departures, all found in the building.

*A seventh, found after S5 shipped: the reshapes were fast and their OUTPUT was not.* The family's whole
argument for being production-visible GDScript is that it runs once per compile over a few hundred
vertices rather than per cell — and that is true, and it is not the same as being free. Path Meanderize
turned a 250-vertex line into ~4000 in 220 ms, exactly as advertised, and the **native** per-cell path
query then took 11.8 s at 128x128 and did not finish at 512x512. `native_supported()` answered true the
whole time; nothing fell back.

The fault was in `Pasture3DPathGeom`, not in any S5 node. Its bucket grid sized the cell from the MEAN
SEGMENT LENGTH, floored at 0.5 m, on the reasoning that a cell should hold about one segment. But a
uniform grid's cost is paid per RING: a query `d` metres from the line walks `d / cell` shells and visits
O((d/cell)²) buckets before the stopping rule can fire, and almost every cell of the terrain is far from
the path. A 13 cm mean segment pinned the cell at the floor and a cell 150 m away walked 300 shells. The
count was already quadratic in distance before S5 — the reshape family only made the constant visible.

Three changes, in `src/pasture_3d_path_query.cpp` and its GDScript twin `pasture3d_graph_path.gd`:
the cell is now sized by the BOUNDING BOX (about one segment per cell by area, so ~sqrt(n) cells span it);
the ring walk starts at the first ring that can intersect the bucket grid at all, instead of counting
half-metres from a query cell that sits far outside it; and the candidate list is no longer de-duplicated
by linear scan, which was O(m²) in exactly the case that was already expensive — `resolve`'s tie rule is
"lowest segment index wins", so it is independent of both order and multiplicity and a segment offered
twice costs one extra distance and changes no answer.

Measured after: 47 ms at 128x128, 558 ms at 512x512. `PathCarveGate` [A] — native against a brute-force
oracle that uses no index at all — is the control that the answers did not move, and `PathShapeGate` [G]
is the new criterion that stops it regressing, stated as a ratio rather than a millisecond budget.

The lesson is written into `PASTURE3D_NODE_ACCELERATION_GUIDE.md` Step 0 alongside the carve-out that
lets these nodes be visible at all: a PATH node's review question is not "is its maths native", it is
"what does its output cost the kernel that consumes it".

*A shared base, `Pasture3DGraphNodePathShape`.* The port shape, the CONST lowering, the zero grid slot
and the kept output instance are identical on all five and on `Path Width`, which was retrofitted onto
it. Written six times that is six chances to allocate a fresh path per call — which produces correct
terrain and silently costs the geometry table's fanout dedup, a thing only an entry count can see.

*A reshape DROPS the road profile.* `alignment` and the `sample_*` arrays describe a specific centreline.
Move the line and they describe somewhere else while `can_grade()` still answers true, so a Road Grade
downstream would grade to the profile of the path this used to be. `moves_the_line()` is the one thing a
subclass must answer honestly; it is true by default, because keeping a stale alignment is silent and
dropping a valid one is visible immediately. `Path Width` is the only false.

*Widths and heights are carried by PROJECTION, not by an index map.* Three of the five know where each
new vertex came from and two do not — Fractalize and Meanderize move vertices off the original line by
design. Projecting each new vertex onto the input polyline and sampling at the arc length it lands on
answers all five with one definition, and answers it the way a reader expects: a meander bend carries the
width of the stretch of river it bulged out of. An empty array stays empty rather than being resized into
zeros, because zeros in a HEIGHT array are sea level, not "no data".

*`Path Smooth`'s window is in VERTICES, against §7.4's own metric rule.* A moving average is defined on
the samples: a window of "8 metres" on a path whose vertices are 40 m apart is a window of one vertex and
does nothing, with no way to see that from the inspector. The metric knob for smoothing is
`Path Resample`'s step, which is what decides what a vertex is worth. Every other parameter in the family
is metric as written — `sigma` in metres, `min_area` in square metres.

*`Path Fractalize`'s enum is `Bias`, not `Orientation`.* `Orientation` is a `@GlobalScope` enum in Godot,
and a local one of that name shadows it in a way the parser reports only from the SUBCLASS, as a type
mismatch against itself. The exported property keeps the spec's name.

*`Path Meanderize` needed a spatial index, and nothing else in the family does.* Loop removal is the one
O(n²) question here, asked on the one node whose vertex count is MULTIPLIED rather than added to: six
iterations at three divisions turns a 70-point line into 51 000, and the naive pairwise scan is 2.6
billion segment tests per cut attempt. The first version was that scan, and it did not fail the gate — it
...
**What S7b built, and the one thing it did not.** `evaluate` gained a route ahead of the whole-graph
native one: `_native_supported_if_staged` asks whether the derive family is the ONLY thing keeping the
graph off the kernel, and `_stage_derives` then walks the evaluation order handing each derive node its
grid inputs — each one produced by a recursive `evaluate` rooted at the upstream node, so a stage that
lowers takes the kernel and a derive feeding another derive is handled by the recursion rather than by a
second mechanism. The whole program is then compiled with the produced paths in the geometry table, the
derive nodes lowering to CONST like every other PATH producer.

`native_supported()` still answers **false** for such a graph, deliberately. It is the off-thread
contract stated at the top of `evaluate`: compile here, solve there. A program compiled without staging
carries a path that was never produced, so "can this be handed to a worker" and "can `evaluate` use the
kernel for this" are different questions, and S7b answers only the second. `PathStagedCompileGate` [D]
pins that rather than leaving it to look like an oversight.

**The staged route does not `store_cache`.** §8.4 drafts [B] as a cache that invalidates when either
stage's inputs change. There is no such cache: `_compute_node_inputs_hash` folds in the graph's
parameters and the input surface, not the FIELD a stage produced, so a key built from it would not
describe what it holds — which is how a stale paint ships (`cache-key-the-output-not-the-input`). The
staged route recomputes, which is slower than it could be and never wrong; S7b's win is the kernel, not
a skip. What [B] measures instead is the thing that IS keyed correctly: the derive's own path memo
re-derives when the first stage's input moves and not when only the remainder's parameters do, with the
terrain moving in both cases.

### 8.5 What S4 built differently

Three departures from §8.2-8.3 as written, all found in the building.

**1. A memoised recursion, not a topological sweep inside `compile_graph_program`.** §8.2 puts the pass
in the compiler. There are five places that want a resolved path — the two compilers
(`compile_graph_program` and the multi-root tap compiler), the folded evaluator, the unfolded oracle, and
the preview taps — reached by four different orderings, so putting the sweep in one of them means the
other four either re-run it or read a stale answer. `_resolved_path_of(ni, inputs_of)` resolves in
dependency order by construction, from whichever caller asks first, and the memo makes the repeat asks
free. The sweep's only real advantage was avoiding repeated work, which the memo already provides.

**2. Both routes had to be changed, not one.** The native compile reaches a path through
`_path_operand_of`; the GDScript evaluator reaches it through a *separate* function, `_path_inputs`, that
also called `path_output()` directly. Changing only the compiler would have made the two evaluators carve
different terrain from one graph — and the `[Dev/GD]` twins that exist to catch exactly that would then
have been comparing the wrong two things. Both now go through the one resolver.

**3. The cache key carries the input's instance id as well as its digest.** §8.3 says
`(input paths' content_digest, this node's revision)`. A content digest cannot distinguish an input that
was *edited* from one that was *replaced by a different path with the same geometry* — and since a filter
keeps and mutates its own output instance (which it must, or the geometry table's instance-keyed fanout
dedup breaks), that distinction is the one that decides whether the kept instance is still the right
answer. One extra integer in the hash.

**4. `Path Width`'s parameter is `half_width`, not `width`.** §7.3 drafted it as `width`.
`Pasture3DGraphPath.half_widths` and `Pasture3DSpline.half_width` are both half-widths, and a second unit
for the same quantity one hop away is how a river ends up twice the size it was asked for. The floor is
`min_half_width` for the same reason. It also gained `SCALE` semantics for a widthless path — an empty
`half_widths` reads as 1.0 everywhere, so scaling one gives the same answer `SET` would, which is the
only reading of "scale nothing" that is not a surprise.

**5. A PATH producer names no geometry-table entry.** Found by `PathPrePassGate` [B], which is the only
reason it was found at all: the fanout property held — two carves behind one filter did share one entry —
and the table still carried two. `_compile_geometry` walks every slot in the compiled order and asks
`_path_operand_of` what path feeds it, and a filter HAS a path input, so the filter's own slot named the
path coming IN. That entry can never be read: a PATH producer's grid slot is a zero placeholder and its
op lowers to CONST. It was built, deduplicated apart from the path the filter PRODUCES, and uploaded to
the GPU as a second polyline nothing dispatches against — one wasted entry per filter, and the terrain
identical either way, which is why only a count could see it. Slots whose `output_port_type()` is PATH
now name `-1`.
...
Nine departures, all found by building it and by `SplineBrushPresetGate`'s controls.
...
**6. "Has a preset" means "has a Path Carve", not "has a graph".** The first version asked whether the
node carried a `Pasture3DNodeGraph` at all. The toolbar's `_apply_placement_defaults` gives every
`Pasture3DPlow` a Mountain Cone graph before it enters the tree, and a Ridge is a Plow — so every Ridge
placed from the toolbar arrived carrying somebody else's graph, was judged already set up, and got no
child spline and no Path Carve. The placement default now skips presets outright, AND both `_has_preset`
and `_install_preset` ask about the Path Carve specifically, so a brush that already carries a stack gets
its own graph beside it rather than instead of it. Neither fix alone is enough: the first is about this
one caller, the second is about every future one.

Either half alone still counts, deliberately — detach the child spline and the Ridge stops carving rather
than growing a new one on the next load.

**7. `fit_tolerance`, because the fitted loop is a shape people edit.** §9.4 does not mention how many
control points the fit lands with. At the tolerance the first version hard-coded, a 360 m ring came back
with 138 of them — one every two and a half metres, which is worse to drag than to redraw. It is exported
as a tolerance rather than as a point spacing because an offset outline is mostly straight and a straight
run needs two points however long it is. It also widens the loop by the same amount, and that is the
mechanism rather than a side effect: a simplification can only cut inward, so the contour is taken at
`spline_margin + fit_tolerance` and cut back by at most `fit_tolerance`, which is what keeps
`spline_margin` a promise. Default 4 m.

**8. `install_preset_now()` and `adopt_preset_line()`, for callers that build a preset in code.**
...
**9. The presets ship `Path Carve.width_source = CONSTANT`, against the node's own default of PATH.**
Requested by the user after living with departure 3's presets in a real scene, and it is the same
argument one step further on. PATH is the better tool — it is what makes a river widen downstream — but
its width lives on the CHILD spline, a node the user has not met yet when they place their first Ridge.
A preset whose reach is governed from somewhere the user cannot see does not read as configurable, it
reads as broken, which is precisely the first impression departure 3 exists to prevent. So the reach is
`Path Carve.width` (20 m), sitting in the inspector beside the offset that uses it, and switching to PATH
stays a one-click move for anybody who wants the taper. The rest of the user's suggested defaults land
with it: `slope_angle` 30°, `falloff` 10 m, `width_scale` 1.0 on both; Ridge FIXED_WIDTH with no flat
top, Trough SLOPE_ANGLE over a 4 m floor.

The migration has to follow, or the change silently eats authored data. §9.5's table sent old `width` /
`bank_width` to the child spline's `half_width`, which under CONSTANT the carve no longer reads — a
migrated Ridge would have kept its gizmo and taken the preset's 20 m. `_migrate_width` now writes both
(they are the same quantity: `reach` in `src/pasture_3d_path_carve.cpp` is `half_width_at(s)` on one
branch and `params.width` on the other). And an old `width_curve` moves the carve BACK to PATH — a user
who authored a per-vertex taper has already answered the question CONSTANT is the default answer to.
...
| **S7b** ✅ | The staged compile (§8.4): cut the program at each grid-reading geometry node, evaluate, produce the path, compile the remainder. Removes S7a's graph-wide bail. BUILT — `native_supported()` still refuses and the staged route keeps no cache, both for stated reasons; see the end of §8.4. | `PathStagedCompileGate`: [A] a graph with a derive node runs the STAGED route — asserted through `staged_compile_count`, not inferred from the terrain — and matches the S7a reference; control: `force_no_staged_compile` keeps that reference route live, so the comparison has two routes and not one measured twice; [B] the path re-derives when the first stage's input moves and NOT when only the remainder's parameters do, with the terrain moving in both cases; [C] the derive still contributes — 93 m against the same graph without it — because two routes that both ignore the drape agree perfectly; [D] `native_supported()` still refuses, which is the off-thread contract and not a leftover. |
