...
#ifndef PASTURE3D_ASSETS_CLASS_H
#define PASTURE3D_ASSETS_CLASS_H
...
class Pasture3D;
class Pasture3DInstancer;

class Pasture3DAssets : public Resource {
	GDCLASS(Pasture3DAssets, Resource);
	CLASS_NAME();
...
private:
	...
	TypedArray<Pasture3DTextureAsset> _texture_list;
	TypedArray<Pasture3DMeshAsset> _mesh_list;
	...
	void _set_asset_list(const AssetType p_type, const TypedArray<Pasture3DAssetResource> &p_list);
	void _set_asset(const AssetType p_type, const int p_id, const Ref<Pasture3DAssetResource> &p_asset);
...
public:
	Pasture3DAssets() {}
	~Pasture3DAssets() { destroy(); }
	void initialize(Pasture3D *p_terrain);
	bool is_initialized() { return _terrain != nullptr; }
	void uninitialize();
	void destroy();

	void set_texture_asset(const int p_id, const Ref<Pasture3DTextureAsset> &p_texture);
	Ref<Pasture3DTextureAsset> get_texture_asset(const int p_id) const;
	void set_texture_list(const TypedArray<Pasture3DTextureAsset> &p_texture_list);
	TypedArray<Pasture3DTextureAsset> get_texture_list() const { return _texture_list; }
	...
	void set_mesh_asset(const int p_id, const Ref<Pasture3DMeshAsset> &p_mesh_asset);
	Ref<Pasture3DMeshAsset> get_mesh_asset(const int p_id) const;
	void set_mesh_list(const TypedArray<Pasture3DMeshAsset> &p_mesh_list);
	TypedArray<Pasture3DMeshAsset> get_mesh_list() const { return _mesh_list; }
...
VARIANT_ENUM_CAST(Pasture3DAssets::AssetType);

inline Ref<Pasture3DTextureAsset> Pasture3DAssets::get_texture_asset(const int p_id) const {
	if (p_id >= 0 && p_id < _texture_list.size()) {
		return _texture_list[p_id];
	}
	return Ref<Pasture3DTextureAsset>();
}

inline Ref<Pasture3DMeshAsset> Pasture3DAssets::get_mesh_asset(const int p_id) const {
	if (p_id >= 0 && p_id < _mesh_list.size()) {
		return _mesh_list[p_id];
	}
	return Ref<Pasture3DMeshAsset>();
}

#endif // PASTURE3D_ASSETS_CLASS_H
