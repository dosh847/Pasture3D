...
#ifndef PASTURE3D_INSTANCER_CLASS_H
#define PASTURE3D_INSTANCER_CLASS_H
...
class Pasture3D;
class Pasture3DAssets;

class Pasture3DInstancer : public Object {
	GDCLASS(Pasture3DInstancer, Object);
	CLASS_NAME();
	friend Pasture3D;
...
private:
	...
	// MM Resources stored in Pasture3DRegion::_instances as
	...
	int _get_master_lod(const Ref<Pasture3DMeshAsset> &p_ma);
	void _process_updates();
	void _update_mmi_by_region(const Pasture3DRegion *p_region, const int p_mesh_id);
	void _set_mmi_lod_ranges(RID p_mmi, const Ref<Pasture3DMeshAsset> &p_ma, const int p_lod);
	...
	void _backup_region(const Ref<Pasture3DRegion> &p_region);
...
public:
	...
	void clear_by_region(const Ref<Pasture3DRegion> &p_region, const int p_mesh_id);
	...
	void append_region(const Ref<Pasture3DRegion> &p_region, const int p_mesh_id, const TypedArray<Transform3D> &p_xforms,
			const PackedColorArray &p_colors, const bool p_update = true);
	void update_transforms(const AABB &p_aabb);
	int get_closest_mesh_id(const Vector3 &p_global_position) const;
	void copy_paste_dfr(const Pasture3DRegion *p_src_region, const Rect2i &p_src_rect, const Pasture3DRegion *p_dst_region);
...
using InstancerMode = Pasture3DInstancer::InstancerMode;
VARIANT_ENUM_CAST(Pasture3DInstancer::InstancerMode);

// Allows us to instance every X function calls for sparse placement
// Modifies _density_counter, not const!
inline uint32_t Pasture3DInstancer::_get_density_count(const real_t p_density) {
	uint32_t count = 0;
...
inline int Pasture3DInstancer::_get_master_lod(const Ref<Pasture3DMeshAsset> &p_ma) {
...
#endif // PASTURE3D_INSTANCER_CLASS_H