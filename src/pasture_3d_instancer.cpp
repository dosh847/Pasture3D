...
// Creates MMIs based on Multimesh data stored in Pasture3DRegions
void Pasture3DInstancer::_process_updates() {
	if (_queued_updates.empty()) {
		if (RS->is_connected("frame_pre_draw", callable_mp(this, &Pasture3DInstancer::_process_updates))) {
			LOG(DEBUG, "Disconnect from RS::frame_pre_draw signal");
			RS->disconnect("frame_pre_draw", callable_mp(this, &Pasture3DInstancer::_process_updates));
		}
		return;
	}
	IS_DATA_INIT(VOID);
...
void Pasture3DInstancer::_update_mmi_by_region(const Pasture3DRegion *p_region, const int p_mesh_id) {
	if (!p_region) {
		LOG(ERROR, "p_region is null");
		return;
	}
	if (p_mesh_id < 0 || p_mesh_id >= Pasture3DAssets::MAX_MESHES) {
	...
	Ref<Pasture3DMeshAsset> ma = _terrain->get_assets()->get_mesh_asset(p_mesh_id);
	if (!ma.is_valid()) {
	...
	for (const Vector2i &cell : cell_locations) {
		...
		for (int lod = 0; lod < Pasture3DMeshAsset::MAX_LOD_COUNT; lod++) {
		...
		if (shadow_lod_disabled) {
			_destroy_mmi_by_cell(region_loc, p_mesh_id, cell, Pasture3DMeshAsset::SHADOW_LOD_ID);
		...
		for (int lod = ma->get_last_lod(); lod >= Pasture3DMeshAsset::SHADOW_LOD_ID; lod--) {
			// Don't create shadow MMI if not needed
			if (lod == Pasture3DMeshAsset::SHADOW_LOD_ID && shadow_lod_disabled) {
				continue;
			}
			...
			if (modified || !mm.is_valid() || lod == Pasture3DMeshAsset::SHADOW_LOD_ID) {
				...
				if (lod == Pasture3DMeshAsset::SHADOW_LOD_ID) {
		...
		if (ma->get_shadow_impostor() > 0) {
			Vector2i mesh_key(p_mesh_id, Pasture3DMeshAsset::SHADOW_LOD_ID);
			CellMMIDict &cell_mmi_dict = mesh_mmi_dict[mesh_key];
			RID &mmi = cell_mmi_dict[cell].first;
			if (mmi.is_valid()) {
				RS->instance_set_custom_aabb(mmi, mm_custom_aabb);
...
void Pasture3DInstancer::_set_mmi_lod_ranges(RID p_mmi, const Ref<Pasture3DMeshAsset> &p_ma, const int p_lod) {
	...
	if (source_lod == Pasture3DMeshAsset::SHADOW_LOD_ID) {
...
void Pasture3DInstancer::_update_vertex_spacing(const real_t p_vertex_spacing) {
	IS_DATA_INIT(VOID);
	...
	for (int r = 0; r < region_locations.size(); r++) {
		...
		Pasture3DRegion *region = _terrain->get_data()->get_region_ptr(region_loc);
		if (!region) {
			LOG(WARN, "Errant null region found at: ", region_loc);
			continue;
		}
...
void Pasture3DInstancer::_destroy_mmi_by_mesh(const int p_mesh_id) {
...
void Pasture3DInstancer::_destroy_mmi_by_location(const Vector2i &p_region_loc, const int p_mesh_id) {
...
void Pasture3DInstancer::_destroy_mmi_by_cell(const Vector2i &p_region_loc, const int p_mesh_id, const Vector2i p_cell, const int p_lod) {
	if (_mmi_rids.count(p_region_loc) == 0) {
		return;
	}
	MeshMMIDict &mesh_mmi_dict = _mmi_rids[p_region_loc];
	...
	for (int lod = Pasture3DMeshAsset::SHADOW_LOD_ID; lod < Pasture3DMeshAsset::MAX_LOD_COUNT; lod++) {
...
void Pasture3DInstancer::_backup_region(const Ref<Pasture3DRegion> &p_region) {
	if (p_region.is_null()) {
		return;
	}
...
RID Pasture3DInstancer::_create_multimesh(const int p_mesh_id, const int p_lod, const TypedArray<Transform3D> &p_xforms, const PackedColorArray &p_colors) const {
	RID mm;
	IS_INIT(mm);
	if (p_xforms.size() == 0) {
		return mm;
	}
	Ref<Pasture3DMeshAsset> mesh_asset = _terrain->get_assets()->get_mesh_asset(p_mesh_id);
...
Vector2i Pasture3DInstancer::_get_cell(const Vector3 &p_global_position, const int p_region_size) const {
	IS_INIT(V2I_ZERO);
	real_t vertex_spacing = _terrain->get_vertex_spacing();
	Vector2i cell;
...
Array Pasture3DInstancer::_get_usable_height(const Vector3 &p_global_position, const Vector2 &p_slope_range, const bool p_on_collision, const real_t p_raycast_start) const {
	IS_DATA_INIT(Array());
	const Pasture3DData *data = _terrain->get_data();
...
void Pasture3DInstancer::initialize(Pasture3D *p_terrain) {
	if (p_terrain) {
		_terrain = p_terrain;
	}
	IS_DATA_INIT_MESG("Pasture3D not initialized yet", VOID);
	LOG(INFO, "Initializing Instancer");
	update_mmis();
}

void Pasture3DInstancer::destroy() {
	IS_DATA_INIT(VOID);
	_queued_updates.clear();
...
void Pasture3DInstancer::clear_by_location(const Vector2i &p_region_loc, const int p_mesh_id) {
	LOG(INFO, "Deleting Multimeshes w/ mesh_id: ", p_mesh_id, " in region: ", p_region_loc);
	Ref<Pasture3DRegion> region = _terrain->get_data()->get_region(p_region_loc);
	clear_by_region(region, p_mesh_id);
}

void Pasture3DInstancer::clear_by_region(const Ref<Pasture3DRegion> &p_region, const int p_mesh_id) {
	if (p_region.is_null()) {
...
void Pasture3DInstancer::set_mode(const InstancerMode p_mode) {
...
void Pasture3DInstancer::add_instances(const Vector3 &p_global_position, const Dictionary &p_params) {
	...
	Ref<Pasture3DMeshAsset> mesh_asset = _terrain->get_assets()->get_mesh_asset(mesh_id);
...
void Pasture3DInstancer::remove_instances(const Vector3 &p_global_position, const Dictionary &p_params) {
	IS_DATA_INIT_MESG("Instancer isn't initialized.", VOID);
	...
	for (const Vector2i &region_loc : region_queue) {
		...
		for (int m = (modifier_shift ? 0 : mesh_id); m <= (modifier_shift ? mesh_count - 1 : mesh_id); m++) {
			...
			Ref<Pasture3DMeshAsset> mesh_asset = _terrain->get_assets()->get_mesh_asset(m);
...
void Pasture3DInstancer::add_multimesh(const int p_mesh_id, const Ref<MultiMesh> &p_multimesh, const Transform3D &p_xform, const bool p_update) {
...
void Pasture3DInstancer::add_transforms(const int p_mesh_id, const TypedArray<Transform3D> &p_xforms, const PackedColorArray &p_colors, const bool p_update) {
	IS_DATA_INIT_MESG("Instancer isn't initialized.", VOID);
	if (p_xforms.size() == 0) {
		return;
	}
...
void Pasture3DInstancer::append_location(const Vector2i &p_region_loc, const int p_mesh_id,
...
void Pasture3DInstancer::append_region(const Ref<Pasture3DRegion> &p_region, const int p_mesh_id,
		const TypedArray<Transform3D> &p_xforms, const PackedColorArray &p_colors, const bool p_update) {
	if (p_region.is_null()) {
...
void Pasture3DInstancer::update_transforms(const AABB &p_aabb) {
	IS_DATA_INIT_MESG("Instancer isn't initialized.", VOID);
	...
	for (const Vector2i &region_loc : region_queue) {
		...
		for (const int &region_mesh_id : mesh_types) {
			...
			Ref<Pasture3DMeshAsset> mesh_asset = _terrain->get_assets()->get_mesh_asset(region_mesh_id);
			real_t mesh_height_offset = mesh_asset->get_height_offset();
...
int Pasture3DInstancer::get_closest_mesh_id(const Vector3 &p_global_position) const {
	...
	const Pasture3DRegion *region = _terrain->get_data()->get_region_ptr(region_loc);
	if (!region) {
...
void Pasture3DInstancer::copy_paste_dfr(const Pasture3DRegion *p_src_region, const Rect2i &p_src_rect, const Pasture3DRegion *p_dst_region) {
	...
	for (const int &mesh_id : mesh_types) {
		...
		append_region(Ref<Pasture3DRegion>(p_dst_region), mesh_id, xforms, colors, false);
	}
}
...
void Pasture3DInstancer::swap_ids(const int p_src_id, const int p_dst_id) {
	IS_DATA_INIT_MESG("Instancer isn't initialized.", VOID);
	Ref<Pasture3DAssets> assets = _terrain->get_assets();
	...
	if (p_src_id >= 0 && p_src_id < mesh_count && p_dst_id >= 0 && p_dst_id < mesh_count) {
		TypedArray<Vector2i> region_locations = _terrain->get_data()->get_region_locations();
		for (const Vector2i &region_loc : region_locations) {
			Ref<Pasture3DRegion> region = _terrain->get_data()->get_region(region_loc);
			if (region.is_null()) {
...
void Pasture3DInstancer::update_mmis(const int p_mesh_id, const Vector2i &p_region_loc, const bool p_rebuild) {
	if (_mode == DISABLED) {
	...
	if (p_rebuild) {
		...
		if (!RS->is_connected("frame_pre_draw", callable_mp(this, &Pasture3DInstancer::_process_updates))) {
			...
			RS->connect("frame_pre_draw", callable_mp(this, &Pasture3DInstancer::_process_updates));
		}
		return;
	}
	...
	int mesh_id = CLAMP(p_mesh_id, -1, Pasture3DAssets::MAX_MESHES - 1);
...
void Pasture3DInstancer::_bind_methods() {
	BIND_ENUM_CONSTANT(NORMAL);
	//BIND_ENUM_CONSTANT(PLACEHOLDER);
	BIND_ENUM_CONSTANT(DISABLED);

	ClassDB::bind_method(D_METHOD("clear_by_mesh", "mesh_id"), &Pasture3DInstancer::clear_by_mesh);
	ClassDB::bind_method(D_METHOD("clear_by_location", "region_location", "mesh_id"), &Pasture3DInstancer::clear_by_location);
	ClassDB::bind_method(D_METHOD("clear_by_region", "region", "mesh_id"), &Pasture3DInstancer::clear_by_region);
	ClassDB::bind_method(D_METHOD("set_mode", "mode"), &Pasture3DInstancer::set_mode);
	ClassDB::bind_method(D_METHOD("get_mode"), &Pasture3DInstancer::get_mode);
	ClassDB::bind_method(D_METHOD("is_enabled"), &Pasture3DInstancer::is_enabled);
	ClassDB::bind_method(D_METHOD("add_instances", "global_position", "params"), &Pasture3DInstancer::add_instances);
	ClassDB::bind_method(D_METHOD("remove_instances", "global_position", "params"), &Pasture3DInstancer::remove_instances);
	ClassDB::bind_method(D_METHOD("add_multimesh", "mesh_id", "multimesh", "transform", "update"), &Pasture3DInstancer::add_multimesh, DEFVAL(Transform3D()), DEFVAL(true));
	ClassDB::bind_method(D_METHOD("add_transforms", "mesh_id", "transforms", "colors", "update"), &Pasture3DInstancer::add_transforms, DEFVAL(PackedColorArray()), DEFVAL(true));
	ClassDB::bind_method(D_METHOD("append_location", "region_location", "mesh_id", "transforms", "colors", "update"), &Pasture3DInstancer::append_location, DEFVAL(true));
	ClassDB::bind_method(D_METHOD("append_region", "region", "mesh_id", "transforms", "colors", "update"), &Pasture3DInstancer::append_region, DEFVAL(true));
	ClassDB::bind_method(D_METHOD("update_transforms", "aabb"), &Pasture3DInstancer::update_transforms);
	ClassDB::bind_method(D_METHOD("get_closest_mesh_id", "global_position"), &Pasture3DInstancer::get_closest_mesh_id);
	ClassDB::bind_method(D_METHOD("update_mmis", "mesh_id", "region_location", "rebuild_all"), &Pasture3DInstancer::update_mmis, DEFVAL(-1), DEFVAL(V2I_MAX), DEFVAL(false));
	ClassDB::bind_method(D_METHOD("swap_ids", "src_id", "dest_id"), &Pasture3DInstancer::swap_ids);
