...
#ifndef PASTURE3D_MESH_ASSET_CLASS_H
#define PASTURE3D_MESH_ASSET_CLASS_H
...
#include "constants.h"
#include "pasture_3d_asset_resource.h"
 ...
 * Loaded Assets go through Pasture3DAssets::set_mesh_list(), _set_asset_list()
 * New Assets go through Pasture3DAssets::set_mesh_asset(), _set_asset()
 * Both call initialize() with a new/loaded flag.
 *
 * New assets might be loaded from the API, AssetDock, or Pasture3DAssets::update_mesh_list()
 * when the mesh list is empty.
 */

class Pasture3DMeshAsset : public Pasture3DAssetResource {
	GDCLASS(Pasture3DMeshAsset, Pasture3DAssetResource);
	CLASS_NAME();
...
VARIANT_ENUM_CAST(Pasture3DMeshAsset::GenType);

inline TypedArray<Mesh> Pasture3DMeshAsset::_get_meshes() const {
...
#endif // PASTURE3D_MESH_ASSET_CLASS_H
