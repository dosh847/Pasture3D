...
#ifndef PASTURE3D_COLLISION_CLASS_H
#define PASTURE3D_COLLISION_CLASS_H
...
class Pasture3DCollision : public Object {
	GDCLASS(Pasture3DCollision, Object);
	CLASS_NAME();
...
using CollisionMode = Pasture3DCollision::CollisionMode;
VARIANT_ENUM_CAST(Pasture3DCollision::CollisionMode);

inline Vector2i Pasture3DCollision::_snap_to_grid(const Vector2i &p_pos) const {
	return Vector2i(int_round_mult(p_pos.x, int32_t(_shape_size)),
			int_round_mult(p_pos.y, int32_t(_shape_size)));
}

inline Vector2i Pasture3DCollision::_snap_to_grid(const Vector3 &p_pos) const {
	return Vector2i(Math::floor(p_pos.x / real_t(_shape_size) + 0.5f),
				   Math::floor(p_pos.z / real_t(_shape_size) + 0.5f)) *
			_shape_size;
}

#endif // PASTURE3D_COLLISION_CLASS_H