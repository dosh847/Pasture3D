...
void Pasture3DAssets::_swap_ids(const AssetType p_type, const int p_src_id, const int p_dst_id) {
	...
	Ref<Pasture3DAssetResource> res_src = list[p_src_id];
	if (res_src.is_null()) {
	...
	Ref<Pasture3DAssetResource> res_dst = list[dst_id];
	if (res_dst.is_valid()) {
 ...
 */
void Pasture3DAssets::_set_asset_list(const AssetType p_type, const TypedArray<Pasture3DAssetResource> &p_list) {
	Array list;
	int max_size;
	switch (p_type) {
		case TYPE_TEXTURE:
	...
	for (int i = 0; i < array_size; i++) {
		Ref<Pasture3DAssetResource> res = p_list[i];
		if (res.is_null()) {
		...
		if (!res->is_connected("id_changed", callable_mp(this, &Pasture3DAssets::_swap_ids))) {
			LOG(DEBUG, "Connecting to id_changed, ID: ", id);
			res->connect("id_changed", callable_mp(this, &Pasture3DAssets::_swap_ids));
		}
		res->initialize();
	}
	if (Pasture3D::debug_level >= DEBUG) {
		for (int i = 0; i < list.size(); i++) {
			Ref<Pasture3DAssetResource> res = list[i];
...
void Pasture3DAssets::_set_asset(const AssetType p_type, const int p_id, const Ref<Pasture3DAssetResource> &p_asset) {
	...
	if (p_asset.is_null()) {
		...
		if (id == list.size() - 1) {
			...
			// Else just clear it
			Ref<Pasture3DAssetResource> res = list[id];
			res->clear();
			res->_id = id;
		}
	} else {
		...
		if (!p_asset->is_connected("id_changed", callable_mp(this, &Pasture3DAssets::_swap_ids))) {
			LOG(DEBUG, "Connecting to id_changed");
			p_asset->connect("id_changed", callable_mp(this, &Pasture3DAssets::_swap_ids));
		}
		p_asset->initialize();
	}
}

void Pasture3DAssets::_update_texture_files() {
	IS_INIT(VOID);
	...
	for (const Ref<Pasture3DTextureAsset> &ta : _texture_list) {
		if (ta.is_null()) {
			continue;
		}
...
void Pasture3DAssets::_update_texture_settings() {
...
void Pasture3DAssets::_update_mesh(const int p_id) {
	if (p_id < 0 || p_id >= _mesh_list.size()) {
		LOG(ERROR, "Invalid mesh ID: ", p_id);
		return;
	}
	Ref<Pasture3DMeshAsset> ma = _mesh_list[p_id];
	create_mesh_thumbnails(p_id, V2I(512), true);
	IS_INSTANCER_INIT(VOID);
	Pasture3DInstancer *instancer = _terrain->get_instancer();
	instancer->update_mmis(p_id, V2I_MAX, false);
}

void Pasture3DAssets::_setup_thumbnail_creation() {
	IS_INIT(VOID);
...
void Pasture3DAssets::initialize(Pasture3D *p_terrain) {
	if (p_terrain) {
		_terrain = p_terrain;
	} else {
...
void Pasture3DAssets::uninitialize() {
	LOG(INFO, "Uninitializing assets");
	_terrain = nullptr;
}

void Pasture3DAssets::destroy() {
...
void Pasture3DAssets::set_texture_asset(const int p_id, const Ref<Pasture3DTextureAsset> &p_texture) {
...
void Pasture3DAssets::set_texture_list(const TypedArray<Pasture3DTextureAsset> &p_texture_list) {
...
void Pasture3DAssets::clear_textures(const bool p_update) {
...
void Pasture3DAssets::update_texture_list() {
	...
	for (const Ref<Pasture3DTextureAsset> &ta : _texture_list) {
		...
		if (!ta->is_connected("file_changed", callable_mp(this, &Pasture3DAssets::_update_texture_files))) {
			LOG(DEBUG, "Connecting file_changed signal");
			ta->connect("file_changed", callable_mp(this, &Pasture3DAssets::_update_texture_files));
		}
		if (!ta->is_connected("setting_changed", callable_mp(this, &Pasture3DAssets::_update_texture_settings))) {
			LOG(DEBUG, "Connecting setting_changed signal");
			ta->connect("setting_changed", callable_mp(this, &Pasture3DAssets::_update_texture_settings));
		}
	}
...
// Called when creating a new asset
void Pasture3DAssets::set_mesh_asset(const int p_id, const Ref<Pasture3DMeshAsset> &p_mesh_asset) {
...
// Called when loading a list of assets from an assets resource file on disk
void Pasture3DAssets::set_mesh_list(const TypedArray<Pasture3DMeshAsset> &p_mesh_list) {
...
void Pasture3DAssets::create_mesh_thumbnails(const int p_id, const Vector2i &p_size, const bool p_force) {
	...
	for (int i = start; i < end; i++) {
		Ref<Pasture3DMeshAsset> ma = get_mesh_asset(i);
		LOG(EXTREME, i, ": Getting Pasture3DMeshAsset: ", ptr_to_str(*ma));
		if (ma.is_null()) {
			LOG(ERROR, i, ": Pasture3DMeshAsset is null, skipping");
			continue;
		}
...
void Pasture3DAssets::update_mesh_list() {
	IS_INSTANCER_INIT(VOID);
	...
	if (_mesh_list.size() == 0) {
		...
		Ref<Pasture3DMeshAsset> new_ma;
		new_ma.instantiate();
		set_mesh_asset(0, new_ma);
	}
	LOG(DEBUG, "Reconnecting mesh instance signals");
	for (Ref<Pasture3DMeshAsset> ma : _mesh_list) {
		if (ma.is_null()) {
			LOG(ERROR, "Null Pasture3DMeshAsset found at index ", _mesh_list.find(ma));
			continue;
		}
		if (ma->get_mesh().is_null()) {
			LOG(ERROR, "Pasture3DMeshAsset has null mesh at index ", _mesh_list.find(ma));
			continue;
		}
		if (!ma->is_connected("instancer_setting_changed", callable_mp(this, &Pasture3DAssets::_update_mesh))) {
			LOG(DEBUG, "Connecting instancer_setting_changed signal to _update_mesh");
			ma->connect("instancer_setting_changed", callable_mp(this, &Pasture3DAssets::_update_mesh));
		}
	}
...
Error Pasture3DAssets::save(const String &p_path) {
...
void Pasture3DAssets::_bind_methods() {
	BIND_ENUM_CONSTANT(TYPE_TEXTURE);
	BIND_ENUM_CONSTANT(TYPE_MESH);
	BIND_CONSTANT(MAX_TEXTURES);
	BIND_CONSTANT(MAX_MESHES);

	ClassDB::bind_method(D_METHOD("set_texture_asset", "id", "texture"), &Pasture3DAssets::set_texture_asset);
	ClassDB::bind_method(D_METHOD("get_texture_asset", "id"), &Pasture3DAssets::get_texture_asset);
	ClassDB::bind_method(D_METHOD("set_texture_list", "texture_list"), &Pasture3DAssets::set_texture_list);
	ClassDB::bind_method(D_METHOD("get_texture_list"), &Pasture3DAssets::get_texture_list);
	ClassDB::bind_method(D_METHOD("get_texture_count"), &Pasture3DAssets::get_texture_count);
	ClassDB::bind_method(D_METHOD("get_albedo_array_rid"), &Pasture3DAssets::get_albedo_array_rid);
	ClassDB::bind_method(D_METHOD("get_normal_array_rid"), &Pasture3DAssets::get_normal_array_rid);
	ClassDB::bind_method(D_METHOD("get_texture_colors"), &Pasture3DAssets::get_texture_colors);
	ClassDB::bind_method(D_METHOD("get_texture_normal_depths"), &Pasture3DAssets::get_texture_normal_depths);
	ClassDB::bind_method(D_METHOD("get_texture_ao_strengths"), &Pasture3DAssets::get_texture_ao_strengths);
	ClassDB::bind_method(D_METHOD("get_texture_ao_light_affects"), &Pasture3DAssets::get_texture_ao_light_affects);
	ClassDB::bind_method(D_METHOD("get_texture_roughness_mods"), &Pasture3DAssets::get_texture_roughness_mods);
	ClassDB::bind_method(D_METHOD("get_texture_uv_scales"), &Pasture3DAssets::get_texture_uv_scales);
	ClassDB::bind_method(D_METHOD("get_texture_detiles"), &Pasture3DAssets::get_texture_detiles);
	ClassDB::bind_method(D_METHOD("get_texture_displacements"), &Pasture3DAssets::get_texture_displacements);
	ClassDB::bind_method(D_METHOD("clear_textures", "update"), &Pasture3DAssets::clear_textures, DEFVAL(false));
	ClassDB::bind_method(D_METHOD("update_texture_list"), &Pasture3DAssets::update_texture_list);

	ClassDB::bind_method(D_METHOD("set_mesh_asset", "id", "mesh"), &Pasture3DAssets::set_mesh_asset);
	ClassDB::bind_method(D_METHOD("get_mesh_asset", "id"), &Pasture3DAssets::get_mesh_asset);
	ClassDB::bind_method(D_METHOD("set_mesh_list", "mesh_list"), &Pasture3DAssets::set_mesh_list);
	ClassDB::bind_method(D_METHOD("get_mesh_list"), &Pasture3DAssets::get_mesh_list);
	ClassDB::bind_method(D_METHOD("get_mesh_count"), &Pasture3DAssets::get_mesh_count);
	ClassDB::bind_method(D_METHOD("create_mesh_thumbnails", "id", "size", "force"), &Pasture3DAssets::create_mesh_thumbnails, DEFVAL(-1), DEFVAL(V2I(512)), DEFVAL(false));
	ClassDB::bind_method(D_METHOD("update_mesh_list"), &Pasture3DAssets::update_mesh_list);

	ClassDB::bind_method(D_METHOD("save", "path"), &Pasture3DAssets::save, DEFVAL(""));

	int ro_flags = PROPERTY_USAGE_STORAGE | PROPERTY_USAGE_EDITOR | PROPERTY_USAGE_READ_ONLY;
	ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "mesh_list", PROPERTY_HINT_ARRAY_TYPE, "Pasture3DMeshAsset", ro_flags), "set_mesh_list", "get_mesh_list");
	ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "texture_list", PROPERTY_HINT_ARRAY_TYPE, "Pasture3DTextureAsset", ro_flags), "set_texture_list", "get_texture_list");
